...
(function() {
    ...
    var appInit = function() {
        require([
            ...
            "dojox/html/entities"
        ], function(dojo, dijit, dom, parser, router, registry, topic, request, rest, connect) {
            parser.parse();
    ...
    var _enableSecurity = function(connect) {
        ...
        connect.subscribe("/dojo/io/done", function(/*dojo.Deferred*/ dfd, /*Object*/ response){
            return dfd.then(function(response) {
                if (response && response.result && response.result === "licenseExpired") {
                    registry.byId("appToaster").setContent(response.message, "error");
                ...
                if (dfd.ioArgs.xhr.status === 401) {
                    ...
                    window.location = core.getContextPath()+"/";
                }
                
                if (dfd.ioArgs.xhr.status === 402) {
                    window.location = core.getContextPath()+"/";
                }
            });
    ...
    var route = {
        ...
        'execute': function() {
            ...
            if (currentRouterEvent) {
                ...
                dojo.publish("/FogPanel/event/route/changed", window.location.hash);
            }
            else {
    ...
    var core = {
        ...
        'getSingleSignOnURL': function() {
            return singleSignOnUrl.baseURL;
        },
        ...
        'setSingleSignOnUrl': function(url) {
            singleSignOnUrl.baseURL = url;
        },
