    ...
    'populateValues': function() {
        ...
        usageInfo.query().then(function(data) {
            dojo.forEach(data, function(usageData) {
                dojo.byId("totalDaily").innerHTML = localeCurrency.format(usageData.daily);
                dojo.byId("totalMonthly").innerHTML = localeCurrency.format(usageData.currMonthCost);
                dojo.byId("totalPayment").innerHTML = localeCurrency.format(usageData.currMonthPayment);
...
//                currency = accountData.currency;
            ...
            if (data.length > 0) {
                dojo.forEach(data.slice(0, 5), function(accountData) {
                    ...
                    cell3.innerHTML = accountData.currency  + " " + localeCurrency.format(accountData.amount);
                });
            ...
            dojo.byId("creditAccTotal").innerHTML = localeCurrency.format(totalAccount);
            var roundAmt = Math.round((totalAmount) * 100) / 100
            dojo.byId("exAmount").innerHTML = localeCurrency.format(roundAmt);
...
//                currency = accountData.currency;
            ...
            if (data.length > 0) {
                dojo.forEach(data.slice(0, 5), function(accountData) {
                    ...
                    cell1.innerHTML = accountData.accountId;
                    cell2.innerHTML = accountData.account;
                    cell3.innerHTML = accountData.currency + " " + localeCurrency.format(Math.round((accountData.amount) * 100) / 100);
                });
            ...
            var roundAmt =  Math.round((totalAmount) * 100) / 100;
            dojo.byId("payAmount").innerHTML = localeCurrency.format(roundAmt);
    ...
    'loadIncomeForecast': function() {
        ...
        incomeForecastStore.query().then(function(data) {
            dojo.forEach(data, function(incomeData) {
                dojo.byId("preMonth").innerHTML = "    " + incomeData.currency + " " + localeCurrency.format(incomeData.preMonth);
                dojo.byId("quat").innerHTML = "    " +incomeData.currency + " "+ localeCurrency.format(incomeData.qut);
                dojo.byId("half").innerHTML = "    " + incomeData.currency + " " + localeCurrency.format(incomeData.half);
                dojo.byId("annual").innerHTML ="    " +  incomeData.currency + " " + localeCurrency.format(incomeData.year);
            });
        });
    },
...
Chart, Pie, Tom, Tooltip, MoveSlice, Default, Lines, Magnify, theme, ColumnsPlot, Highlight, connect, timing, Notification, localeCurrency) {
