...
require([
    ...
    "dijit/Dialog"
        ...
        DataGrid, EnhancedGrid, ContentPane, Source, MultiSelect, localeCurrency, dom, win) {
...
var VMBandwidthInfo = {
    ...
    'populateValues': function() {
        ...
        var vmBandDataLayout = [[
                ...
                {'name': translator.common.message.usedBandwidth, 'field': 'totalBand', 'width': '200px', datatype: "string", autoComplete: true},
                ...
                {'name': translator.common.template.grid.cost + " ("+ core.getCurrency() +")", 'field': 'exBandCost', 'width': '100%', datatype: "string", autoComplete: true, 'formatter': function(data) {
                ...
                if (el.vmBandItems.length == 0 || el.vmBandItems == undefined || el.vmBandItems == "undefined" || el.vmBandItems == '' || el.vmBandItems == 'null' || el.vmBandItems == null) {
                    ...
                    dojo.forEach(el.vmBandItems, function(vmBand) {
                        vmBandDataList.newItem({
                            ...
                            exBandCost: localeCurrency.format(vmBand.bandCost),
            ...
            dojo.forEach(data, function(resultData) {
                if (resultData.result == "OK") {
                    var vmStartJobStatus = setInterval(function() {
                        InstancesInfo.vmJob(resultData.jobId, vmStartJobStatus, currentVMId, translator.common.instance.vmRestartSuccessMessage);
                    }, 3000);
    ...
    startVm: function(currentVMId) {
        ...
        instanceStartRestStore.add({
            ...
            dojo.forEach(data, function(resultData) {
                if (resultData.result == "OK") {
                    var vmStartJobStatus = setInterval(function() {
                        InstancesInfo.vmJob(resultData.jobId, vmStartJobStatus, currentVMId, translator.common.instance.vmStartedSuccessMessage);
                    }, 3000);
                    var index = dijit.byId("adminInstanceDataGrid").selection.selectedIndex;
                    var item = dijit.byId("adminInstanceDataGrid").getItem(index);
                    var store = dijit.byId("adminInstanceDataGrid").store;
            ...
            dojo.forEach(data, function(resultData) {
                if (resultData.result == "OK") {
                    var vmStopJobStatus = setInterval(function() {
                        InstancesInfo.vmJob(resultData.jobId, vmStopJobStatus, currentVMId, translator.common.instance.vmStoppedSuccessMessage);
                    }, 3000);
                    var index = dijit.byId("adminInstanceDataGrid").selection.selectedIndex;
                    var item = dijit.byId("adminInstanceDataGrid").getItem(index);
                    var store = dijit.byId("adminInstanceDataGrid").store;
