...
require([
    ...
    ItemFileWriteStore, DataGrid, ContentPane, Source, MultiSelect, dom, win, localeCurrency) {             
        window.query = query;
...
//                DraftInvoiceInfo.populateValues();       
            ...
            "serviceCostChange" : action(function() {
                core.ui.loadTemplate("serviceCostChange", core.ui.getContentId());
                ServiceCostChange.init();
            }),
            
        });
    });

var ServiceCostChange = {
    
     'init' : function() {
                  
        if(dijit.byId("serviceCostChangeGridWidget")) {
            dijit.byId("serviceCostChangeGridWidget").destroyRecursive();
        }
        
        var histortGridData = {
            items: []
        };
        dojo.byId("serviceCostChangeList").innerHTML = "<img src='images/vmload.gif' alt='Loading' height='36' width='100'/> </br> <p>"+translator.common.loader.loading+"</p>";
          
        var serviceChangeLogRest = new JsonRest({
            target: core.getContextPath()+"/api/report/serviceChangeLog"
        });
         
        var histortGridDataList = new ItemFileWriteStore({data: histortGridData});
        var histortGridLayout = [[
            {'name': 'Id', 'field': 'id', 'hidden': 'true'},
            {'name': translator.common.serviceName, 'field': 'serviceName', 'width': '120px', 'datatype':'string',  'autoComplete': 'true','formatter': function(data) {                         
                                             
                                return  "<span class='bold'>" + data + "</span>";
            }},
            {'name': translator.common.itemName, 'field': 'itemName', 'width': '200px', 'datatype':'string',  'autoComplete': 'true'},
            {'name': translator.common.changedBy, 'field': 'changedBy', 'width': '100px', 'datatype':'string',  'autoComplete': 'true' ,'formatter': function(data) {                         
                                             
                                return  "<span class='bold'>" + data + "</span>";
            }},
            {'name': translator.common.zone, 'field': 'zone', 'width': '200px', 'datatype':'string',  'autoComplete': 'true'},
            {'name': translator.common.oldCost + " ("+ core.getCurrency() +")", 'field': 'oldCost', 'width': '100px', 'datatype':'string',  'autoComplete': 'true','formatter': function(data) {                         
                                             
                                return  "<span class='greenColor'>" + data + "</span>";
            }},
            {'name': translator.common.newCost + " ("+ core.getCurrency() +")", 'field': 'newCost', 'width': '120px', 'datatype':'string',  'autoComplete': 'true','formatter': function(data) {                         
                                             
                                return  "<span class='orangeColor'>" + data + "</span>";
            }},
            {'name': translator.common.unit, 'field': 'unit', 'width': '120px', 'datatype':'string',  'autoComplete': 'true'},
            {'name': translator.common.changedDate, 'field': 'changedDate', 'width': '100%', 'datatype':'string',  'autoComplete': 'true'},                        
        ]
    ];  
         
         
         serviceChangeLogRest.query().then(function(data) {
           if(data.length === 0 || data === undefined || data === 'undefined' || data === "" || data === " ") {
                dojo.byId("serviceCostChangeList").innerHTML  = "";
                dojo.byId("noServiceCostChangeMessageBox").style.display = "block";       
            } else {
                dojo.byId("noServiceCostChangeMessageBox").style.display = "none";       
                dojo.forEach(data, function(itemData) {   
                    histortGridDataList.newItem({
                        id: itemData.id,
                        serviceName: itemData.serviceName,
                        zone: itemData.zone,
                        itemName: itemData.itemName,
                        changedBy: itemData.user,
                        oldCost: itemData.oldCost,
                        newCost: itemData.newCost,
                        changedDate: itemData.changedDate,
                        unit:itemData.unit
                    });
                });
                dojo.byId("serviceCostChangeList").innerHTML  = "";
                var serviceCostChangeGrid = new EnhancedGrid({
                    id: 'serviceCostChangeGridWidget',
                    store: histortGridDataList,
                    structure: histortGridLayout,
                    autoHeight: true,
                    plugins: core.getGridConfig().plugins
                });
                serviceCostChangeGrid.placeAt("serviceCostChangeList");
                serviceCostChangeGrid.startup();
            }  
        });         
     }    
};
    ...
    populateValues : function() {
        ...
        this._customRestStore.query().then(function(data) {
            dojo.forEach(data, function(resultData) {
                dojo.byId("recAmt").innerHTML = localeCurrency.format(resultData.recurringItemsTotalAmount);
                dojo.byId("cusAmt").innerHTML =  localeCurrency.format(resultData.customItemsTotalAmount);                
        ...
        var gridLayout = [[
            ...
            {'name': translator.common.amount + " ("+ core.getCurrency() +")", 'field': 'amount', 'width': '100px', 'datatype':'string',  'autoComplete': 'true'},
            ...
            {'name': translator.common.taxAmount + " ("+ core.getCurrency() +")", 'field': 'taxAmount', 'width': '100px', 'datatype':'string',  'autoComplete': 'true'},
            {'name': translator.common.billing.grid.layout.totalAmount + " ("+ core.getCurrency() +")", 'field': 'totalAmount', 'width': '100px', 'datatype':'string',  'autoComplete': 'true', 'formatter': function(data) {                         
            ...
            if(resultData.customItemList.length == 0 || resultData.customItemList == undefined || resultData.customItemList == 'undefined' || resultData.customItemList == "" || resultData.customItemList == " ") {
                ...
                dojo.forEach(resultData.customItemList, function(data) {
                    customDataList.newItem({
                        id: data.id,
                        taxAmount: localeCurrency.format(data.taxAmount),
                        totalAmount: localeCurrency.format(Math.round(data.totalAmount * 100) / 100),
                        amount: data.amount,
    ...
    populateValues : function() {
        ...
        var taxDataLayout = [
            [
                ...
                {'name': translator.common.billing.grid.layout.currentUsage + " ("+ core.getCurrency() +")", 'field': 'currentDue', 'width': '100px', 'datatype':'string',  'autoComplete': 'true'},
        ...
        this._invoiceCountStore.query({status:"DRAFT"}).then(function(data) {             
            if(data[0].invoiceList.length == 0 || data[0].invoiceList == undefined || data[0].invoiceList == 'undefined' || data[0].invoiceList == "" || data[0].invoiceList == " ") {
                ...
                dojo.forEach(data, function(resultData) {
                    dojo.byId("daily").innerHTML = resultData.currency + " " + localeCurrency.format(Math.round((resultData.daily) * 100) / 100) ;                            
                    dojo.byId("monthly").innerHTML = resultData.currency + " " +  localeCurrency.format(Math.round((resultData.monthly) * 100) / 100) ;
                    dojo.byId("preMonth").innerHTML = resultData.currency + " " + localeCurrency.format(Math.round((resultData.previousMonth) * 100) / 100);
            ...
            dojo.forEach(data, function(resultData) {
                ...
                dojo.byId("daily").innerHTML =  localeCurrency.format(Math.round((resultData.daily) * 100) / 100);                            
                dojo.byId("monthly").innerHTML =  localeCurrency.format(Math.round((resultData.monthly) * 100) / 100); 
                dojo.byId("preMonth").innerHTML =  localeCurrency.format(Math.round((resultData.previousMonth) * 100) / 100);
...
var PaymentInfo = {
    ...
    this._paymentCountRestStore.query({month: "Today"}).then(function(data) {
        dojo.forEach(data, function(resultData) {            
            dojo.byId("daily").innerHTML = localeCurrency.format(Math.round((resultData.dailyPaymentAmount) * 100) / 100);
            dojo.byId("preMonth").innerHTML =  localeCurrency.format(Math.round((resultData.preMonthPaymentAmount) * 100) / 100);
            dojo.byId("monthly").innerHTML =   localeCurrency.format(Math.round((resultData.currentMonthPaymentAmount) * 100) / 100);           
            ...
            if(resultData.paymentItems.length == 0 || resultData.paymentItems == undefined || resultData.paymentItems == 'undefined' || resultData.paymentItems == "" || resultData.paymentItems == " ") {
                ...
                dojo.forEach(resultData.paymentItems, function(paymentData) {
                    paymentDataList.newItem({
                        ...
                        totalAmount: localeCurrency.format(paymentData.totalAmount * 100),
