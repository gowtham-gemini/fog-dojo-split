...
ItemFileWriteStore, Memory, DataGrid, ContentPane, Source, MultiSelect, dom, win, localeCurrency) {
    ...
    },
    {
        ...
        "emailTemplate": action(function() {
            core.ui.loadTemplate("emailTemplate", core.ui.getContentId());
            EmailTemplateConfig.init();
            EmailTemplateConfig.populateValues();
        }),
        ...
        "retail": action(function() {
            ...
            if (dijit.byId("retailCreditLimitEditConformationLimitDialog")) {
                dijit.byId("retailCreditLimitEditConformationLimitDialog").destroyRecursive();
            }
        ...
        "emailTemplateContentArea": action(function(id) {
            var emailTemplateContentForm = dijit.byId("emailTemplateContentForm");
            if (emailTemplateContentForm) {
                dijit.byId("emailTemplateContentForm").destroyRecursive();
            }
            core.ui.loadTemplate("emailTemplateContentArea", core.ui.getContentId());
            EmailTemplateContentInfo.init();
            EmailTemplateContentInfo.populateValues(id);
        ...
        "language": action(function() {
            ...
            if (dijit.byId("languageUpdateConfirmationdialog")) {
                dijit.byId("languageUpdateConfirmationdialog").destroyRecursive();
            }            
...
var LanguageConfig = {
    ...
    'showUpdateConfirmationDialog': function() {
        dijit.byId("languageUpdateConfirmationdialog").show();
    },
    'update': function() {        
        var languageStore = new JsonRest({
            target: core.getContextPath() + "/api/config/language"
        });
        dijit.byId("languageUpdateConfirmationdialog").hide();
        languageStore.add({
...
//                        dojo.byId("retailCreditLimitDescription").innerHTML = config.description;
                } else if (config.name == "retail.enableThresholdLimit") {
                    if (config.value === "true" || config.value === true) {
                        dojo.byId("enableThresholdLimitDefaultValue").style.display = "block";
                        dijit.byId("enableThresholdLimit").setAttribute('checked', true);
                    } else {
                        dojo.byId("enableThresholdLimitDefaultValue").style.display = "none";
                        dijit.byId("enableThresholdLimit").setAttribute('checked', false);
                    }                       
                } else if (config.name == "retail.instance.limit") {
                    dijit.byId("retailInstanceLimit").setValue(config.value);
                } else if (config.name == "retail.network.limit") {
                    dijit.byId("retailNetworkLimit").setValue(config.value);
                } else if (config.name == "retail.storage.limit") {
                    dijit.byId("retailStorageLimit").setValue(config.value);
                } else if (config.name == "retail.snapshot.limit") {
                    dijit.byId("retailSnapshotLimit").setValue(config.value);
                } else if (config.name == "retail.public.ip.limit") {
                    
                    dijit.byId("retailPublicIPLimit").setValue(config.value);

                } else if (config.name == "retail.vpc.limit") {
                    dijit.byId("retailVpcLimit").setValue(config.value);

                } else if (config.name == "retail.cpu.limit") {
                    dijit.byId("retailCpuLimit").setValue(config.value);

                } else if (config.name == "retail.memory.limit") {
                    dijit.byId("retailMemoryLimit").setValue(config.value);

                } else if (config.name == "retail.primary.storage.limit") {
                    dijit.byId("primaryStorageLimit").setValue(config.value);

                } else if (config.name == "retail.secondary.storage.limit") {
                    dijit.byId("secondaryStorageLimit").setValue(config.value);
    ...
    closeUpdate: function() {
        ...
        dijit.byId("retailCreditLimitEditConformationLimitDialog").hide();
    },
    updateShow: function() {
                ...
                dojo.forEach(resultData, function(config) {
                    if (config.name == "creditcard.processing") {
                        if (config.value == "false" || config.value == false) {
                            ...
                            dijit.byId("retailCreditLimitEditConformationLimitDialog").show();
...
//            RetailManagement.update();
    ...
    update: function() {
        ...
        var retailCreditLimit = dijit.byId("retailCreditLimit");
         ...
         var enableThresholdLimit = dijit.byId("enableThresholdLimit");
         var retailInstanceLimit = dijit.byId("retailInstanceLimit");
         var retailStorageLimit = dijit.byId("retailStorageLimit");
        ...
        if (firstNode) {
            ...
            this._creditConfigStore.add({
                ...
                enableThresholdLimit:enableThresholdLimit.checked,
                retailInstanceLimit:retailInstanceLimit.value,
                retailStorageLimit:retailStorageLimit.value,
                retailSnapshotLimit:retailSnapshotLimit.value,
                retailPublicIPLimit:retailPublicIPLimit.value,
                retailVpcLimit:retailVpcLimit.value,
                retailCpuLimit:retailCpuLimit.value,
                retailMemoryLimit:retailMemoryLimit.value,
                primaryStorageLimit:primaryStorageLimit.value,
                secondaryStorageLimit:secondaryStorageLimit.value,
                retailNetworkLimit:retailNetworkLimit.value
...
var GeneralConfig = {
    ...
    enableThresholdLimitChange : function(data) {
        
        if (data.checked === true || data.checked === "true") {
            dojo.byId("enableThresholdLimitDefaultValue").style.display = "block";
            dojo.byId("resoueceThresould").style.display = "block";
            
            dijit.byId("retailInstanceLimit").setAttribute("required", true);
            dijit.byId("retailStorageLimit").setAttribute("required", true);
            dijit.byId("retailSnapshotLimit").setAttribute("required", true);
            dijit.byId("retailPublicIPLimit").setAttribute("required", true);
            dijit.byId("retailVpcLimit").setAttribute("required", true);
            dijit.byId("retailCpuLimit").setAttribute("required", true);
            dijit.byId("retailMemoryLimit").setAttribute("required", true);
            dijit.byId("primaryStorageLimit").setAttribute("required", true);
            dijit.byId("secondaryStorageLimit").setAttribute("required", true);
            
            this._configRestStore.query().then(function(resultData) {
            dojo.forEach(resultData, function(config) {
                 if (config.name == "retail.instance.limit") {
                    dijit.byId("retailInstanceLimit").setValue(config.value);
                } else if (config.name == "retail.storage.limit") {
                    dijit.byId("retailStorageLimit").setValue(config.value);
                } else if (config.name == "retail.snapshot.limit") {
        ...
        } else if (data.checked === false || data.checked === false) {
            dojo.byId("enableThresholdLimitDefaultValue").style.display = "none";
            dojo.byId("resoueceThresould").style.display = "none";
            
             dijit.byId("retailInstanceLimit").setAttribute("required", false);
            dijit.byId("retailStorageLimit").setAttribute("required", false);
            dijit.byId("retailSnapshotLimit").setAttribute("required", false);
            dijit.byId("retailPublicIPLimit").setAttribute("required", false);
            dijit.byId("retailVpcLimit").setAttribute("required", false);
            dijit.byId("retailCpuLimit").setAttribute("required", false);
            dijit.byId("retailMemoryLimit").setAttribute("required", false);
            dijit.byId("primaryStorageLimit").setAttribute("required", false);
            dijit.byId("secondaryStorageLimit").setAttribute("required", false);
...
var EmailTemplateConfig = {
    _mailTemplateRestStore: "",
    init: function() {
        this._mailTemplateRestStore = new JsonRest({
            target: core.getContextPath() + "/api/mailTemplate"
        });
    },
    populateValues: function() {
        this._mailTemplateRestStore.query().then(function(data) {
            dojo.forEach(data, function(resultData) {
...
var EmailTemplateContentInfo = {
    ...
    populateValues: function(currentId) {
        if (currentId == 5 || currentId == 6) {
            dojo.byId("emailTemplateTitle").style.display = "none";
            dojo.byId("emailTemplateSubject").style.display = "none";
            dojo.byId("emailTemplateHeader").style.display = "none";
            dojo.byId("emailTemplateFooter").style.display = "none";
        } else {
            dojo.byId("emailTemplateTitle").style.display = "block";
            dojo.byId("emailTemplateSubject").style.display = "block";
            dojo.byId("emailTemplateHeader").style.display = "block";
            dojo.byId("emailTemplateFooter").style.display = "block";
        }
        this._currentId = currentId;
        var templateName = dijit.byId("templateTilte");
        var templateSubject = dijit.byId("templateSubject");
        var templateContent = dijit.byId("emailTemplateContent");

        var includeHeader = dijit.byId("includeHeader");
//       var includeSignature = dijit.byId("includeSignature");
        var includeFooter = dijit.byId("includeFooter");
        this._mailTemplateRestStore.get(currentId).then(function(result) {
            dojo.byId("emailTemplateName").innerHTML = result.title;
            dojo.byId("emailTempNameRefer").value = result.name;
            templateName.setValue(result.title);
            templateSubject.setValue(result.subject);
            templateContent.setValue(result.description);
            includeHeader.set("checked", result.hasHeader)
//           includeSignature.set("checked", result.hasSignature)
            includeFooter.set("checked", result.hasFooter)

        })
    },
    update: function() {
        var templateTilte = dijit.byId("templateTilte");
        var templateSubject = dijit.byId("templateSubject");
        var templateContent = dijit.byId("emailTemplateContent");
        var includeHeader = dijit.byId("includeHeader");
//       var includeSignature = dijit.byId("includeSignature");
        var includeFooter = dijit.byId("includeFooter");
        dijit.byId("emailTempButton").setAttribute('style', 'display: none');
        dojo.byId("emailTempLoader").style.display = "block";
        this._mailTemplateRestStore.put({
            id: Number(this._currentId),
            title: templateTilte.getValue(),
            subject: templateSubject.getValue(),
            content: templateContent.getValue(),
            hasHeader: includeHeader.checked,
            hasFooter: includeFooter.checked
//           hasSignature: includeSignature.checked

        }).then(function(result) {
            dojo.forEach(result, function(resultData) {
                dijit.byId("emailTempButton").setAttribute('style', 'display: block');
                dojo.byId("emailTempLoader").style.display = "none";
                if (resultData.Result == "OK") {
                    dojo.byId("emailTemplateName").innerHTML = resultData.title;
                    registry.byId('appToaster').setContent(translator.common.message.updatedSuccess, 'message');
                    registry.byId('appToaster').show();
                } else {
                    registry.byId('appToaster').setContent(translator.common.message.failed, 'error');
    ...
    sendMail: function() {
        var sendMailRestStore = new JsonRest({
            target: core.getContextPath() + "/api/mailTemplate/sendEmail"
        });
        sendMailRestStore.add({
            email: "nandhini@assistanz.com",
            templateName: dojo.byId("emailTempNameRefer").value
        });
...
var PaymentGatewayList = {
    init: function() {
        ...
        var gridLayout = [[
                ...
                {'name': translator.common.procFeeAmount+ " ("+ core.getCurrency() +")", 'field': 'procFeeAmount', 'width': '100%', datatype: "number", autoComplete: true}
            ]];
            ...
            dojo.forEach(data, function(gatewayList) {
                ...
                dojo.forEach(gatewayList.paymentsGateways, function(gatewayList) {
                    gridDataList.newItem({
                        ...
                        procFeeAmount: localeCurrency.format(gatewayList.processingFeeAmount),
...
window.EmailTemplateConfig = EmailTemplateConfig;
window.EmailTemplateContentInfo = EmailTemplateContentInfo;
