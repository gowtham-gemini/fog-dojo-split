    ...
    populateValues : function () {
        ...
        var gridLayout = [
            [   
                ...
                {'name': translator.user.userName, 'field': 'userName', 'width': '300px', datatype:"string",  autoComplete: true},
                {'name': translator.common.name, 'field': 'name', 'width': '250px', datatype:"string",  autoComplete: true},  
...
var AddInvitationInfo = {
    populateValues : function () {
        var multipleInvitationWidget = new FogPanel.MultipleInvitation({
            onEmailWidgetChange : function (currentWidget, widget) {
                 AddInvitationInfo.validateEmail(currentWidget, widget);
            }
        })
    ...
    validateEmail : function (currentEmail, currentWidget) {         
        ...
        dojo.forEach(widget, function (el) {                                                  
            var ValidateInvitationRestStore = new JsonRest({
                target: core.getContextPath()+"/api/account/invitation/email/validate"
            });
            ValidateInvitationRestStore.query({email: currentEmail.value}).then(function (data) {  
                currentEmail.validator = function () {
                    if(data[0].result === "invalid") {
                        currentEmail.set("invalidMessage", translator.common.invalidEmail);   
                        dijit.byId("addInvitationButton").set("disabled", true);
                        return false;
                    } else if(data[0].result === "exist")  {
                        dijit.byId("addInvitationButton").set("disabled", true);
                        currentEmail.set("invalidMessage",  translator.common.emailExist);
                        return false;
                    } else if(el.getFieldsValue().email === currentEmail.value &&  el.id !== currentWidget.id) {
                        dijit.byId("addInvitationButton").set("disabled", true);                        
                        currentEmail.set("invalidMessage", translator.common.duplicateEmail);
                        return false;
                    } else {
    ...
    sendInvitation : function () {
        ...
        sendInvitationRestStore.add({
            ...
            if(resultData[0].result === "OK") {
                ...
                registry.byId("appToaster").setContent(translator.common.duplacateEmailMsg, "error");
                registry.byId("appToaster").show();
            } else {
                registry.byId("appToaster").setContent(translator.common.cloudStack.errorMessage,"error");
                registry.byId("appToaster").show();
    ...
    addInvitationConfirm : function () {                   
        ...
        dojo.forEach(widget, function (el, count) {                                          
            ...
            ValidateInvitationRestStore.query({email: el.getFieldsValue().email}).then(function (data) {  
                var currentEmail = el.getCurrentEmailWidget();                  
                if(data[0].result === "invalid") {                     
                    currentEmail.set("invalidMessage", translator.common.invalidEmail);   
            ...
            dojo.forEach(widget, function (element, index) {
                if(el.getFieldsValue().email === element.getFieldsValue().email && count !== index) {
                    dojo.query("#invitationErrorMsg").removeClass("hide_text", true);
                    el.getCurrentEmailWidget().set("invalidMessage",  translator.common.emailExist);
                    stat = false;  
                } 
            });
        });          
    ...
    populateValues: function() {
        ...
        var recurringDataLayout = [
            [
                ...
                {'name': translator.common.amount + " ("+ core.getCurrency() +")", 'field': 'amount', 'width': '150px'},
                {'name': translator.common.billing.taxPercentage, 'field': 'taxPercentage', 'width': '100px'},
                {'name': translator.common.taxAmount + " ("+ core.getCurrency()+")", 'field': 'taxAmount', 'width': '100px'},
                {'name': translator.common.billing.grid.layout.totalAmount + " ("+ core.getCurrency() +")", 'field': 'totalAmount', 'width': '150px', 'formatter': function(data) {                         
...
var PyamentsInfo = {
    ...
    populateValues: function() {
        ...
        var paymentDataLayout = [[
                ...
                {'name': translator.common.amount + "("+ core.getCurrency() +")", 'field': 'amount', 'width': '100px'},
                {'name': translator.common.billing.processingFee + "("+ core.getCurrency() +")", 'field': 'processingFee', 'width': '100px'},
                {'name': translator.common.billing.grid.layout.totalAmount + "("+ core.getCurrency() +")", 'field': 'totalAmount', 'width': '100px', 'formatter': function(data) {                                                                      
                        return  "<span class='orangeColor'>" + data + "</span>";
                    }
                },
    ...
    populateValues: function() {
        ...
        var taxDataLayout = [
            [
                ...
                {'name': translator.common.billing.grid.layout.currentUsage + " ("+ core.getCurrency()  +")", 'field': 'currentDue', 'width': '200px'},
                {'name':  translator.common.billing.grid.layout.totalPayable + " ("+ core.getCurrency()  +")", 'field': 'totalAmount', 'width': '200px', 'formatter': function(data) {                                                                      
                        return  "<span class='orangeColor'>" + data + "</span>";
                    }
                },
    ...
    populateValues: function() {
        ...
        var gridLayout = [
            [
                ...
                {'name': translator.common.billing.currentDue + " (" + core.getCurrency() +")", 'field': 'due', 'width': '150px', datatype:"string",  autoComplete: true, 'formatter': function(data) {                                                                      
                        return  "<span class='orangeColor'>" + data + "</span>";
                    }
                },    
                {'name': translator.common.billing.paid + " (" + core.getCurrency() +")", 'field': 'paid', 'width': '100px', datatype:"string",  autoComplete: true, 'formatter': function(data) {                                                                      
                        return data;
                    }
                },     
