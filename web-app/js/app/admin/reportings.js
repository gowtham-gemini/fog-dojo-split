...
require([
    ...
    },
    {
        ...
        "clientUsage": action(function() {
            if (dijit.byId("clientUsageReportForm")) {
                dijit.byId("clientUsageReportForm").destroyRecursive();
            }
            core.ui.loadTemplate("clientUsageReport", core.ui.getContentId());
            ClientUsageReport.init();
            ClientUsageReport.populateValues();
        ...
        if (formJson.forDateRange == "SELECTIVE") {
            ...
            if (firstNode) {
                ...
                if (formJson.forBillableItem == "SELECTIVE" && (formJson.billableItemList.toString() == "")) {
                    ...
                    document.getElementById("billableItemPdf").setAttribute("href", "" + core.getBaseURL() + "/pdf/pdfLink?pdfController=pdf&pdfAction=billableItemReportNew&type=pdf&fromDate=" + formJson.reportFromDate +
                            "&toDate=" + formJson.reportToDate +
                            "&forDateRange=" + formJson.forDateRange +
                            "&forAccount=" + formJson.forAccount +
                            "&forZone=" + formJson.forZone +
...
//        var toDate = dojo.date.locale.format(formJson.reportToDate, {datePattern: "dd/MM/yyyy", selector: "date"});
        ...
        if (formJson.forDateRange == "SELECTIVE") {
            ...
            if (firstNode) {
                ...
                document.getElementById("creditLimitReportPdf").setAttribute("href", "" + core.getBaseURL() + "/pdf/pdfLink?pdfController=pdf&type=pdf&pdfAction=creditLimitReport&fromDate=" + formJson.reportFromDate +
                        "&toDate=" + formJson.reportToDate +
                        "&forDateRange=" + formJson.forDateRange +
...
//        var toDate = dojo.date.locale.format(formJson.reportToDate, {datePattern: "dd/MM/yyyy", selector: "date"});
        ...
        if (formJson.forDateRange == "SELECTIVE") {
            ...
            if (firstNode) {
                ...
                document.getElementById("paymentPendingReportPdf").setAttribute("href", "" + core.getBaseURL() + "/pdf/pdfLink?pdfController=pdf&pdfAction=paymentPendingReport&type=pdf&fromDate=" + formJson.reportFromDate +
                        "&toDate=" + formJson.reportToDate +
                        "&forDateRange=" + formJson.forDateRange +
...
var ClientUsageReport = {
    ...
    init: function() {
        ...
        currency.query().then(function(data) {
            dojo.forEach(data, function(cur) {
                dojo.byId("clientUsageCurrencyValue").innerHTML = cur.currency;
            });
        });
    },
    populateValues: function() {
    ...
    showDate: function(value) {
        ...
        if (value == "ALL") {
            dojo.byId("clientUsageReportDateRangeDiv").style.display = "none";
        } else {
            dojo.byId("clientUsageReportDateRangeDiv").style.display = "block";
        }
    },
    generate: function() {

        var formJson = JSON.parse(dojo.formToJson("clientUsageReportForm"), true);
        dojo.byId("clientUsageReportActionButtonDiv").style.display = "block";
//        var fromDate = dojo.date.locale.format(formJson.reportFromDate, {datePattern: "dd/MM/yyyy", selector: "date"});
//        var toDate = dojo.date.locale.format(formJson.reportToDate, {datePattern: "dd/MM/yyyy", selector: "date"});
        ...
        if (formJson.forDateRange == "SELECTIVE") {
            ...
            var pageNode = dojo.byId("clientUsageReportDateRangeDiv");
            var widgets = dijit.registry.findWidgets(pageNode);
            var firstNode = null;
            var state = true;
            dojo.forEach(widgets, function(el) {
            ...
            if (firstNode) {
                firstNode.focus();
                document.getElementById("clientUsageReportIframe").src = "";
                dojo.byId("clientUsageReportActionButtonDiv").style.display = "none";
            } else {
                dojo.byId("clientUsageReportActionButtonDiv").style.display = "block";
                document.getElementById("clientUsageReportIframe").src = "pdf/clientUsageReport?fromDate=" + formJson.reportFromDate +
                        "&toDate=" + formJson.reportToDate +
                        "&forStatus=" + formJson.forStatus +
                        "&forDateRange=" + formJson.forDateRange;

                document.getElementById("clientUsageReportCsv").setAttribute("href", "" + core.getBaseURL() + "/pdf/clientUsageReport?format=csv&fromDate=" + formJson.reportFromDate +
                        "&toDate=" + formJson.reportToDate +
                        "&forStatus=" + formJson.forStatus +
                        "&forDateRange=" + formJson.forDateRange);

                document.getElementById("clientUsageReportPdf").setAttribute("href", "" + core.getBaseURL() + "/pdf/pdfLink?pdfController=pdf&pdfAction=clientUsageReport&type=pdf&fromDate=" + formJson.reportFromDate +
                        "&toDate=" + formJson.reportToDate +
                        "&forStatus=" + formJson.forStatus +
                        "&forDateRange=" + formJson.forDateRange +
                        "&filename=client-usage-report");
    ...
    cancel: function() {
        document.getElementById("clientUsageReportIframe").src = "";
        dijit.byId("clientUsageReportForm").reset();
        dojo.byId("clientUsageErrorDiv").style.display = "none";
        dojo.byId("clientUsageReportActionButtonDiv").style.display = "none";
    }
};
...
window.ClientUsageReport = ClientUsageReport;