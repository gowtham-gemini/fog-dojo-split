...
//            VPCMenuInfo.populateValues(currentVPC);
        ...
        "vpcConnection": action(function(id) {
            if (dijit.byId("vpcLoader")) {
                dijit.byId("vpcLoader").destroyRecursive();
            }
            if (dijit.byId("ipVpcTabCointainer")) {
            ...
            setTimeout(function () {
                var mainTab = dijit.byId("ipVpcTabCointainer"); //Tr
                var subIpTab = dijit.byId("vpcVpnConnectionTab"); //tab Id which you want to show
                mainTab.selectChild(subIpTab);   
            },800);
        }),
...
var NetworkTopology = {
                ...
                dojo.forEach(data, function(resultData) {
                    if (data && resultData.result === "OK") {
                        ...
                        registry.byId("userToaster").show();
                         ...
                         NetworkTopology.populateValues();
                         dojo.query("#topoAddTierCloudstackException").toggleClass("hide_text", true);
                         dojo.byId("topologyaddTierCloudstackExceptionMsg").innerHTML = "";
                    } else {
                        registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                        registry.byId("userToaster").show();
                        dijit.byId("topoVpcLoader").hide();
                        dojo.query("#topoAddTierCloudstackException").removeClass("hide_text", true);
                        dojo.byId("topologyaddTierCloudstackExceptionMsg").innerHTML = resultData.message;
                    }
                });
            });
        }
    ...
    addTierShow : function () {     
        ...
        var networkFileStoreList = new ItemFileWriteStore({data: networkOfferOption});  
        var aclRestStoreList = new ItemFileWriteStore({data: option});  
                        ...
                        if(vpcData.appTier.length === 0 || vpcData.appTier === "" || vpcData.appTier === undefined) {
                           ...
                           var tierWidget = new dijit.TitlePane({
                            ...
                            if(appTierSize === index + 1) {
                                tierNode.className = 'tiers-cont-last span12';
                            } else {
                                tierNode.className = 'tiers-cont span12';
                            }
...
var vpcPFDetails = {
    ...
    cancelVMGrid : function () {
        dojo.query("#vpcPFCloudstackException").toggleClass("hide_text", true);
        dojo.byId("vpcPFCloudstackExceptionMsg").innerHTML = "";
                ...
                networkLoadBalancerRestStore.add({ipId: currentIPId.value, networkId: networkId.value,
                    ...
                    dojo.forEach(data, function(resultData) {
                        if (resultData.result === "OK") {
                            ...
                            registry.byId("userToaster").setContent(resultData.message, "error");
                            registry.byId("userToaster").show();
                            dojo.query("#vpcPFCloudstackException").removeClass("hide_text", true);
                            dojo.byId("vpcPFCloudstackExceptionMsg").innerHTML = resultData.message;
                            dijit.byId('userNetworkIPPortForwardingAddForm').reset();
                            dijit.byId("viewIpLoader").hide();                            
                        } else {
                            registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
...
//                    dojo.query("#VMRequireMsg").toggleClass("hide_text", true);
                    ...
                    dijit.byId('viewIpLoader').hide();     
                    dojo.query("#vpcPFCloudstackException").removeClass("hide_text", true);
                    dojo.byId("vpcPFCloudstackExceptionMsg").innerHTML = jobResultData.message;
                }
            });
        });
    },
                ...
                networkLoadBalancerRestStore.add({ipId: currentIPId.value, networkId: networkId.value, algorithm: algorithm.value, name: name.value, privatePort: privatePort.value, publicPort: publicPort.value, vmList: vmListArray.toString()}).then(function(data) {
                    dojo.forEach(data, function(resultData) {
                        if (resultData.result === "OK") {
                            ...
                            dojo.query("#vpcLBCloudstackException").toggleClass("hide_text", true);
                            dojo.byId("vpcLBCloudstackExceptionMsg").innerHTML = "";
                        } else if (resultData.result === "FAILED") {
                            registry.byId("userToaster").setContent(resultData.message, "error");
                            registry.byId("userToaster").show();
                            dojo.query("#vpcLBCloudstackException").removeClass("hide_text", true);
                            dojo.byId("vpcLBCloudstackExceptionMsg").innerHTML = resultData.message;
                            dijit.byId("viewIpLoader").hide();
                        } else {
                            registry.byId("userToaster").setContent(translator.common.message.cloudStackError, "error");
                            registry.byId("userToaster").show();
...
var viewVpcIp = {
    'enableStaticNat': function() {
        ...
        if (firstNode) {
            ...
            if(vmId.value === "option" || networkId.value === "option" || vmId.value === "" || networkId.value === "") {
                 dojo.query("#viewIPenableStaticNatErrorMessage").removeClass("hide_text", true); 
            } else {
                dojo.query("#viewIPenableStaticNatErrorMessage").toggleClass("hide_text", true);             
                dijit.byId('viewIpLoader').show();
        ...
        var tierList = new ItemFileWriteStore({data: tierOptions});  
         ...
         networkRestStore.query({vpcId: dojo.byId("currentVpcId").value}).then(function(data) {            
            if(data.length === 0 || data === "" || data === undefined) {
                 ...
                 dijit.byId("vpcIpPageTier").set("store", noTierList);                 
                 dijit.byId("vpcIpPageTier").set("value", "option");    
             } else {
                 var firstTier = "";
                 dojo.forEach(data, function(tier, index) {
                     if(tier.tierType === "WEB" || tier.tierType === "DB") {
                         tierList.newItem({
                             id: tier.referenceId,
                             name: tier.name,
                         ...
                         if(index === 0) {
                             firstTier = tier.referenceId;
                         }
                     }
                 });  
                 dijit.byId("vpcIpPageTier").set("store", tierList);
                 dijit.byId("vpcIpPageTier").set("value", firstTier);                 
             }                                                          
    ...
    'getTierVMList' : function(data) {              
        if(data.value === "option" || data.value === "" || data.value === undefined) {
            ...
            dijit.byId("vpcIPPageTierVm").set("store", noVmList);
            dijit.byId("vpcIPPageTierVm").set("value", "option");
        } else { 
            var neworkid = "";
            
            data.store.fetch({query: {id: data.value}, onItem: function(item) {
                    neworkid=item.networkId;
                }          
            });  
            ...
            if(neworkid ||  neworkid !== "undefined" || neworkid !== "" || neworkid !== null ) {
                networkVMListRestStore.query({networkId: neworkid}).then(function(data) {                    
                    if(data.length === 0 || data === undefined || data === "" || data[0].name === undefined) {
                        var noOptions = {
                            identifier: 'id',
                            label: 'name',
                            items: [{id: "option" , name : translator.common.selectVM}]                
                        };       
                        var noVMList = new ItemFileWriteStore({data: noOptions});                
                        dijit.byId("vpcIPPageTierVm").set("store", noVMList);
                        dijit.byId("vpcIPPageTierVm").set("value", "option");
                    } else {
                        dojo.forEach(data, function(vm, index) {
                            vmList.newItem({id: vm.referenceId, name: vm.name});
                            if (index === 0) {
...
var ViewVpc = {
    ...
    'enableStaticNat': function() {
        ...
        var pageNode = dojo.byId("vpcInfoIpEnableStaticNatFormPage");
        var widgets = dijit.registry.findWidgets(pageNode);
        var firstNode = null;
        var wigetState = true;
        dojo.forEach(widgets, function(el) {
        ...
        if (firstNode) {
            ...
            if(vmId.value === "option" || vmId.value === "" || networkId.value === "option" || networkId.value === "") {
                dojo.query("#vpcEnableStaticNatErrorMessage").removeClass("hide_text", true); 
            }  else {
                dojo.query("#vpcEnableStaticNatErrorMessage").toggleClass("hide_text", true); 
                dijit.byId('vpcLoader').show();
    ...
    'enableStaticNatShow' : function(id) {       
        ...
        networkRestStore.query({vpcId: dojo.byId("currentVpcId").value}).then(function(data) {
            if(data.length === 0 || data === "" || data === undefined) {
                 ...
                 var noTierList = new ItemFileWriteStore({data: noTierOptions}); 
                 dijit.byId("vpcTier").set("store", noTierList);                 
                 dijit.byId("vpcTier").set("value", "option");    
             } else {                    
                 var firstVal = "";
                 var count = 0;
                 dojo.forEach(data, function(tier) {                 
                 ...
                 dijit.byId("vpcTier").set("value", firstVal);  
             }                        
    ...
    'populateTierValues': function() {
        ...
        dojo.query("#vpcInfoAddTierCloudstackException").toggleClass("hide_text", true);
        dojo.byId("vpcInfoAddTierCloudstackExceptionMsg").innerHTML = "";
                ...
                {'name': translator.common.action, 'field': 'action',                                        
                    'formatter': function(data) {                                                                        
                         var vpnStatus = new FogPanel.VPNStatus({
                           onAclVPN : function () {ViewVpc.replaceACLTierShow(data.referenceId);},
...
//                            cost:  resultData.cost,
    ...
    'acquireIPShow': function() {
        ...
        dojo.byId("vpcIConditionExceptionMsg").style.display = "none";
...
//        alert(agrementChecked)
        if(agrementChecked == true) {                     
            ...
            dojo.byId("vpcIConditionExceptionMsg").style.display = "block";
        }
    },
    ...
    'populateIPListValues': function() {
        ...
        var dataLayout = [[
                ...
                {'name': translator.common.virtualMachine, 'field': 'virtualMachine', 'width': '200px', 'datatype': 'string', 'autoComplete': 'true'},  
...
//    mainTab.selectChild(subIpTab);     
  ...
  'getTierVMList' : function(data) {      
      ...
      if(data.value === "option" || data.value === "" || data.value === undefined) {
          ...
          var noVmList = new ItemFileWriteStore({data: noVMOptions});
          dijit.byId("vpcTierVm").set("store", noVmList);
          dijit.byId("vpcTierVm").set("value", "option");
      } else {
          var vmOptions = {
              identifier: 'id',
              label: 'name',
            ...
            tierRestStore.add({netmask: netmask, acl:acl, name: name, gateway: gateway, networkOffer: networkOffer, vpcId: dojo.byId("currentVpcId").value}).then(function(data) {
                dojo.forEach(data, function(resultData) {
                    if (data && resultData.result === "OK") {
                        ...
                        registry.byId("userToaster").show();
                        dijit.byId("vpcLoader").hide();                        
                        dojo.query("#vpcInfoAddTierCloudstackException").removeClass("hide_text", true);
                        dojo.byId("vpcInfoAddTierCloudstackExceptionMsg").innerHTML = resultData.message;
                    }
                });
            });
        }
            ...
            [
                ...
                {'name': translator.common.name, 'field': 'vpcName', 'width': '200px', 'datatype':'string',  'autoComplete': 'true', 'formatter' : function (data) {
                        ...
                        if(data.state === "Enabled") {
                            html = "<a href='#/user/vpc/view/"+data.referenceId+"'>" + data.name + "</a>";
                        } else {
                            html = data.name;
                        }
                ...
                {'name': translator.common.state, 'field': 'state','width': '100px', 'datatype':'string',  'autoComplete': 'true', 'formatter' : function (stat) {                        
                        var imageDiv = domConstruct.create("div", { innerHTML: "<img src='images/vmload.gif' class='vm_stat_image' alt='"+translator.common.loader.loading+"'/>" });
                        var status;                                                 
                        if(stat === "Enabled") {
                            status = "<div class='stat_running overflowLabel'>"+translator.common.enabled+"</div>";
                        } else if(stat === "Inactive") {
                            status = "<div class='stat_stopped overflowLabel'>"+translator.common.inactive+"</div>";                                
                        } else if(stat === "error" || stat === "Error") {
                            status = "<div class='stat_destroyed overflowLabel'>"+translator.common.error+"</div>";   
                        } else if(stat === "Creating") {
                            status = imageDiv.innerHTML;
                        } else {
                            status = "";
                        }                                                                                                                       
                ...
                {'name': translator.common.action, 'field': 'action','width': '100%', 'datatype':'string',  'autoComplete': 'true', 'formatter' : function (data) {
                       ...
                       } else if(data.state === "error" || data.state === "Error") {
                           vpnStatus = new FogPanel.VPNStatus({
                               onConfigVPN : function () {core.router.go("#/user/vpc/view/"+data.referenceId);},
                               onRebootVPN : function () {ListVpc.showRestartVpc(data.referenceId);},
                               onEditVPN : function () {ListVpc.showEdit(data);},
                            ...
                            vpnStatus.enableOnlyDeleteMode();
                        }                        
                       return vpnStatus;
                    }
                }                
             ...
             if(data.length === 0 || data === undefined || data === "undefined" || data === "" || data === " ") {
                 ...
                 dojo.forEach(data, function(vpcData) {
                     vpcDataList.newItem({
                         ...
                         vpcName:{'name':vpcData.name, 'referenceId':vpcData.referenceId, state: vpcData.state},
                         description: vpcData.description, 
                         zone: vpcData.zone,
                         cidr: vpcData.cidr,
                         state: vpcData.state,                     
                         action : {'referenceId':vpcData.referenceId, 'description':vpcData.description, 'name':vpcData.name, state: vpcData.state}
                     });
                 });
                 dojo.byId("userVpcList").innerHTML = "";
...
var AddVpc = {
  'init': function(currentId) {
        dojo.query("#addVpcCloudstackException").toggleClass("hide_text", true);
        dojo.byId("addVpcCloudstackExceptionMsg").innerHTML = "";
    ...
    'cancel': function() {
        dijit.byId("userCreateVpcForm").reset();
        dojo.query("#addVpcCloudstackException").toggleClass("hide_text", true);
        dojo.byId("addVpcCloudstackExceptionMsg").innerHTML = "";
//        core.router.go("#/user/vpc/list");
    },
            ...
            networkRestStore.add({zone: zone, name: name, desc: desc, cidr: vpcCidr, vpcOffering:vpcOffering,networkDomain:networkDomain}).then(function(data) {
                dojo.forEach(data, function(resultData) {
                    if (data && resultData.result === "OK") {
                        ...
                        registry.byId("userToaster").show();
                        dijit.byId("vpcLoader").hide();
                        dojo.query("#addVpcCloudstackException").removeClass("hide_text", true);
                        dojo.byId("addVpcCloudstackExceptionMsg").innerHTML = resultData.message;
                    }
                });
            });
        }
