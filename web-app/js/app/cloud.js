        ...
        var firstVal = "";
         diskOfferRestStore.query({clusterReferenceId:computClusterId, tags: tag, customized:customWidget, storageType: storageType}).then(function(data) {               
             if(data == "" || data == "" || data.length == 0) { 
                 ...
                 dojo.byId("vmDiskCost").innerHTML = data[0].diskOfferZoneCosts[0].cost;
                 ...
                 dojo.byId("vmDiskMonthlyCost").innerHTML = Math.ceil((data[0].size * Number(data[0].diskOfferZoneCosts[0].cost) * 720.00) * 100.00) / 100.00 + " /" + translator.common.month;
                 dojo.forEach(data, function(el) {
                     diskList.newItem({
                         ...
                         size:el.size,
                         type:el.tags,
                         cost:el.diskOfferZoneCosts[0].cost,
                         minCost:el.diskOfferZoneCosts[0].minimumCost,
                         minSize:el.minSize,
                         maxSize: el.maxSize,
...
//                    dijit.byId("addVolumeDialog").show();
    ...
    setVMDiskSliderSize : function(currentSpinner) {     
        ...
        dojo.byId("vmDiskMonthlyCost").innerHTML = Math.ceil((currentSpinner.value * Number(monDiskCost) * 720.00) * 100.00) / 100.00 + "/" + translator.common.month;
    ...
    getDiskData : function(currentDisk) {        
        ...
        if(currentDisk.value == "option") {            
            ...
            if(item) { 
                ...
                dojo.byId("vmDiskCost").innerHTML = item.cost;
                dojo.byId("vmDetailStorageUnit").innerHTML = "     " + translator.common.gbPerHr;  
                if(item.custom == true || item.custom == "true") {
                    ...
                    dojo.byId("vmDiskMonthlyCost").innerHTML = Math.ceil((dijit.byId("vmStorageDiskSizeSpinner").value * Number(item.cost) * 720.00) * 100.00) / 100.00 + "/" + translator.common.month;
                } else if(item.custom == false || item.custom == "false") {
                    dojo.byId("vmCurrentDiskSize").innerHTML = "  " + item.size + translator.common.gb+",";
                    dojo.byId("vmDiskMonthlyCost").innerHTML = Math.ceil((item.size * Number(item.cost) * 720.00) * 100.00) / 100.00 + "/" + translator.common.month;
                }                 
            }          
        }
        }); 
     ...
     showCustomDisk : function(customCheck) {   
         ...
         diskOfferRestStore.query({clusterReferenceId:computClusterId,tags: tag, customized: customStat, storageType: storageType}).then(function(data) {
            if(data == "" || data == "") {                                                              
                ...
                dojo.byId("vmDiskMonthlyCost").innerHTML = Math.ceil((1 * Number(data[0].diskOfferZoneCosts[0].cost) * 720.00) * 100.00) / 100.00 + "/" + translator.common.month;
...
var CreateVMInfo = {
    ...
    resetTemplate : function () {    
        ...
        if (currentTemplate == "" || currentTemplate == " " || currentTemplate == undefined || currentTemplate == "undefined" || currentTemplate == null || currentTemplate == "null" || templvalNum >= 0) {                        
            var tempOptions = {
                identifier: 'id',            
                label: 'name',
                 ...
                 dojo.forEach(data, function(limitData) {  
                    if(limitData.vmLimit !== "-1") {
                        if((limitData.vmUsed >= limitData.vmLimit)) {
                            ...
                            dojo.byId("createVmButtom").style.display = "none";
                        } 
                    } 
                 });
             });
...
//                            dojo.byId("createVmButtom").style.display = "block";
//                         }
//                     }
//                 });
//             });
...
//            accountListStore.query().then(function(data) {
//                dojo.forEach(data, function(el) {
//                    if(el.accountType == "RETAIL" && el.cardVerified == false) {
...
//                        dijit.byId("customDiskSize").setValue(data[0].minSize);
                        ...
                        dojo.byId("vmDiskOffer").style.display = "block";
//                         dijit.byId("instanceCustomDiskSizeSlider").destroyRecursive();
...
//            var formElements = dojo.query("#userVmActionForm input:checked");
//            var checkedRadioId = dijit.byId(formElements[0].id);        
...
//                        instanceDataList.newItem({
//                            id:vmData.referenceId,
//                            vmName:vmData.name + "," + vmData.referenceId + "," + vmData.osCategory,
//                            plan: vmData.computingOffer, 
//                            zone: vmData.zoneName,
//                            status : vmData.state,
//                            ip: vmData.ip,
...
//            }
        ...
        stopInstance: function() {     
            ...
            instanceStopRestStore.add({
                ...
                dojo.forEach(data, function(resultData) {
                    if(resultData.result == "OK") {
                        var vmStopJobStatus = setInterval(function(){UserInstances.vmJob(resultData.jobId, vmStopJobStatus);},3000);  
        ...
        deleteInstance : function() {
            ...
            instanceDeleteRestStore.add({
                ...
                dojo.forEach(data, function(resultData) {
                    if(resultData.result == "OK") {
                        var vmStopJobStatus = setInterval(function(){UserInstances.vmJob(resultData.jobId, vmStopJobStatus, firewalId);},3000);  
                        var index = dijit.byId("instanceGridWidget").selection.selectedIndex;
                        var item = dijit.byId("instanceGridWidget").getItem(index);
                        var store = dijit.byId("instanceGridWidget").store;                        
                        store.setValue(item, 'status', "in progress");       
                ...
                dojo.forEach(data, function(resultData) {
                    if(resultData.jobResult == "OK") {                        
                        clearInterval(vmJobStatus);                           
                         ...
                         registry.byId("userToaster").show();
//                         securityGroupRestStore.remove(firewalId).then(function(result) {
//                            if(result == true || result == "true") {
//                                registry.byId("userToaster").setContent("Instance and Firewal Has Deleted Successfully!", "message");
//                                registry.byId("userToaster").show();
//                            } else {
////                                registry.byId("userToaster").setContent("Cannot Delete Firewal!", "error");
////                                registry.byId("userToaster").show();
//                            }
//                        });
           ...
           dojo.forEach(currentId, function(el) {
            ...
            instanceStartRestStore.add({
                referenceId:  referenceId
                 ...
                 dojo.forEach(data, function(resultData) {
                    if(resultData.result == "OK") {
                        var vmStartJobStatus = setInterval(function(){UserInstances.vmJob(resultData.jobId, vmStartJobStatus);},3000);  
                        var index = dijit.byId("instanceGridWidget").selection.selectedIndex;
                        var item = dijit.byId("instanceGridWidget").getItem(index);
                        var store = dijit.byId("instanceGridWidget").store;
                        store.setValue(item, 'status', "in progress");
...
//                dojo.forEach(data, function(resultData) {
//                    if(resultData.cardVerified == "OK") {
//                        dijit.byId("instanceZoneWidget").set("value", "option");
//                        dijit.byId("instanceName").reset();
//                        dijit.byId("showVmPage").show();
//                    } else {
//                        core.router.go("#/user/billing/creditCard");  
//                        registry.byId("userToaster").setContent("Card is Not Verified!","error");
//                        registry.byId("userToaster").show();
