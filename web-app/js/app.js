...
//            dijit.byId("dialog").set("content", resultData.value);                         
    ...
    checkEmailFromInvitation : function () {                  
        var email = dijit.byId("newEmail");
        ...
        AccountStore.query({
            ...
            dojo.forEach(data, function(el) {                 
                if (el === "FAILED" || el === "FALSE") {                     
                    if(el === "FAILED") {
                        email.set("invalidMessage", "Email Already Exist");
                        email.set("promptMessage", "Email Already Exist");                        
                    } else {
                        email.set("invalidMessage", "Invalid Email");
                        email.set("promptMessage", "Invalid Email");
                    }
                    email.validator = function() {return false;};
                    email.focus();                      
                }                               
            });
        });
    ...
    validateUserName: function() {
        ...
        AccountStore.query({
            ...
            dojo.forEach(resultData, function(el) {
                if (el) {
                    dojo.byId("userNameStatus").value = el;
                    userName.validator = function() {
                        if (el == "TRUE") {
                            userName.set("invalidMessage", " ");
                            return true;
                        } else if (el == "FAILED") {
                            userName.set("invalidMessage", "User name already exist");
                            return false;
                        } else if (el == "FALSE") {
