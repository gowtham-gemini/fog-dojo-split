...
import com.assistanz.cloud.cloudstack.loadbalancer.LoadBalancerRulesResponse
...
class NetworkService {
    ...
    def listPublicLB(zoneReferenceId, vpcId) {
        try { 
            ...
            for(Iterator <IpAddressResponse> iter = response.ipAddresses.iterator(); iter.hasNext();) { 
                ...
                UserIPAddress userIPAddress = UserIPAddress.findByIpReferenceId(data.publicIpAddressId)
                if(!userIPAddress) {
                    userIPAddress = new UserIPAddress(); 
                    userIPAddress.account = Account.findByUserName(data.publicIpAddressAccount)
                    userIPAddress.user = User.findByUsername(data.publicIpAddressAccount)
                    userIPAddress.ipReferenceId = data.publicIpAddressId
                    userIPAddress.publicIPAddress = data.publicIpAddress
                    userIPAddress.acquireDate = new Date()
                    userIPAddress.virtualMachine = null
                    userIPAddress.network = Network.findByReferenceId(data.publicIpAddressAssociatedNetworkid)
                    userIPAddress.state = data.publicIpAddressState
                    userIPAddress.staticNatVirtualMachine = VirtualMachine.findByReferenceId(data.publicIpAddressVirtualMachineId)
                    userIPAddress.isSourceNat =  Boolean.parseBoolean(data.publicIpAddressIsSourceNat)
                    userIPAddress.isStaticNat = Boolean.parseBoolean(data.publicIpAddressIsStaticNat)
                    userIPAddress.vpc = VPC.findByReferenceId(data.vpcId)
                    if(userIPAddress.isSourceNat == true) {
                        userIPAddress.isFirstIp = true
                    }
                    userIPAddress.zone = Zone.findByReferenceZoneId(data.publicIpAddressZoneId)
            ...
            for(def puplicIp: puplicIpList) {            
                ...
                item.put("account", puplicIp.account?.userName);
    ...
    def listAllLBs(zoneReferenceId, vpcId, scheme) {
        try {
            ...
            HashMap<String,String> optional = new HashMap<String,String>();
                ...
                def data = iter.next();                                                                                                                                               
                def firewallRules = FirewallRules.findByReferenceId(data.lbRuleId)
                def loadBalancer = LoadBalancer.findByReferenceId(data.lbRuleId)
                if(!firewallRules && !loadBalancer) {
                    ...
                    firewallRules.referenceId = data.lbRuleId
                    ...
                    loadBalancer.referenceId = data.lbRuleId
                    loadBalancer.description = data.description
                    loadBalancer.createdDate = new Date()
                    loadBalancer.name = data.loadBalancerName
                    ...
                    for(Iterator <LoadBalancerInstanceResponse> vmlbitter = data.loadBalancerInstances.iterator(); vmlbitter.hasNext();) { 
                        ...
                        def vm = VirtualMachine.findByReferenceId(vmlb.instanceId)
                        
                        def loadbalancervmmap = LoadBalancerVmMap.findAllWhere(loadBalancer : loadBalancer, virtualMachine:VirtualMachine.findByReferenceId(vmlb.instanceId))
                        
                        if(!loadbalancervmmap) {
                            def loadBalancerVmMap = new LoadBalancerVmMap()
                            loadBalancerVmMap.loadBalancer = loadBalancer
                            loadBalancerVmMap.virtualMachine = VirtualMachine.findByReferenceId(vmlb.instanceId)
        ...
        try {
            ...
            for(Iterator <NetworkResponse> iter = response.networks.iterator(); iter.hasNext();) {
                ...
                if(network) {   
                    ...
                    network.trafficType = data.networkTrafficType
                    network.name = data.networkName;
                    network.description = data.networkDisplayText;                    
                    network.networkDomain = data.networkDomain                    
    ...
    def listPortForwardingRules(ipId) {
        ...
        for(Iterator <IpAddressResponse> iter = ipresponse.ipAddresses.iterator(); iter.hasNext();) {
            def data = iter.next();
            def userIpAddress = UserIPAddress.findByIpReferenceId(data.publicIpAddressId)
            userIpAddress.network = Network.findByReferenceId(data.publicIpAddressAssociatedNetworkid)
            userIpAddress.save(flush: true);   
        }        
        ...
        for(Iterator <PortForwardingRules> iter = response.portForwardingRules.iterator(); iter.hasNext();) { 
            ...
            def firewallRules = FirewallRules.findByReferenceId(data.portForwardingRuleId)
            def portForwarding = PortForwarding.findByReferenceId(data.portForwardingRuleId)
            if(!firewallRules && !portForwarding) {
                ...
                firewallRules.account = user.account
                firewallRules.createdDate = new Date()
                firewallRules.referenceId = data.portForwardingRuleId
                ...
                firewallRules.startPort = data.portForwardingRulePublicStartPort
                firewallRules.endPort = data.portForwardingRulePublicEndPort
                firewallRules.state = data.portForwardingRuleState
                firewallRules.protocol = data.portForwardingRuleProtocol
                ...
                portForwarding.referenceId = data.portForwardingRuleId
                portForwarding.createdDate = new Date()
                portForwarding.privateStartPort = data.portForwardingRulePrivateStartPort
                portForwarding.privateEndPort = data.portForwardingRulePrivateEndPort
                portForwarding.virtualMachine = VirtualMachine.findByReferenceId(data.portForwardingRuleVirtualMachineId) 
                ...
                def invoiceItem = InvoiceItem.findWhere(billableItem: pfBillableItem, referenceItemId:data.portForwardingRuleId, invoice:invoice)
    ...
    def listLoadBalancingRules(ipId) {
        ...
        for(Iterator <LoadBalancerRulesResponse> iter = response.loadBalancerRules.iterator(); iter.hasNext();) { 
            def data = iter.next();
            
            def firewallRules = FirewallRules.findByReferenceId(data.loadBalancerId)
            def loadBalancer = LoadBalancer.findByReferenceId(data.loadBalancerId)
            if(!firewallRules && !loadBalancer) {
                ...
                firewallRules.account = user.account
                firewallRules.createdDate = new Date()
                firewallRules.referenceId = data.loadBalancerId
                firewallRules.userIPAddress = UserIPAddress.findByIpReferenceId(ipId)
                ...
                firewallRules.startPort = data.loadBalancerPrivatePort
                firewallRules.endPort = data.loadBalancerPublicPort
                firewallRules.state = data.loadBalancerState
                ...
                loadBalancer = new LoadBalancer()
                loadBalancer.referenceId = data.loadBalancerId
                loadBalancer.createdDate = new Date()
                loadBalancer.name = data.loadBalancerName
                loadBalancer.algorithm = data.loadBalancerAlgorithm
                loadBalancer.privatePort = data.loadBalancerPrivatePort
                loadBalancer.publicPort = data.loadBalancerPublicPort
    ...
    def addLoadBalancingRuleForIp(requestBody) {
        ...
        try {
            ...
            def loadBalancerRuleResponse = loadBalancerServer().assignToLoadBalancerRule(response.loadBalancerId, requestData.vmList)
    ...
    def createNetwork(requestBody) {
        ...
        try {
            ...
            network.referenceId = response.networkId
    ...
    def getNetworkEgressRules(id) {
        ...
        for(Iterator <EgressRuleResponse> iter = response.egressRules.iterator(); iter.hasNext();) {
            def data = iter.next();
            HashMap item = new HashMap();    
            item.put("id", data.id);
    ...
    def getNetworkIpAddressesList(networkId) {
        try {
            ...
            for(Iterator <IpAddressResponse> iter = response.ipAddresses.iterator(); iter.hasNext();) {
                ...
                def isEnabledVPN = RemoteAccessVpn.findByNetwork(Network.findByReferenceId(data.publicIpAddressAssociatedNetworkid))? true: false;
                HashMap item = new HashMap();    
                item.put("id", data.publicIpAddressId);
                item.put("associateNeworkReferenceId", data.publicIpAddressAssociatedNetworkid);
                
                item.put("ip", data.publicIpAddress);
                item.put("account", userIPAddress.account.firstName);                                
                item.put("isSourceNat", data.publicIpAddressIsSourceNat);
                item.put("isStaticNat", data.publicIpAddressIsStaticNat);
                item.put("networkReferenceId", data.publicIpAddressNetworkId);
                item.put("state", data.publicIpAddressState);
                item.put("vmDisplayName", data.publicIpAddressVirtualMachineDisplayName);
                item.put("vmReferenceId", data.publicIpAddressVirtualMachineId);
                item.put("vlanId", data.publicIpAddressVlanId);
                item.put("vlanName", data.publicIpAddressVlanName);
                item.put("zone", Zone.findByReferenceZoneId(data.publicIpAddressZoneId)?.aliasName);
                item.put("vpnEnabled", RemoteAccessVpn.findByPublicIPAddress(UserIPAddress.findByIpReferenceId(data.publicIpAddressId))? true: false);
                item.put("isEnabledVPN", isEnabledVPN);              
                
                item.put("isVPCLBAdded", LoadBalancer.findAllByUserIPAddress(UserIPAddress.findByIpReferenceId(data.publicIpAddressId)) ? true : false);
                item.put("isVPCPFAdded", PortForwarding.findAllByUserIPAddress(UserIPAddress.findByIpReferenceId(data.publicIpAddressId)) ? true : false);
    ...
    def acquireIpForNetwork(requestBody) {
        ...
        try {   
            ...
            if(network.hasFirstIp == true && !invoice) {
                ...
                HashMap item = new HashMap();    
                item.put("result", GeneralConstants.RESULT_SUCCESS);
                item.put("jobId", response.publicIpAddressJobId);
    ...
    def getIpAddressesDetails(id) {
        ...
        for(Iterator <IpAddressResponse> iter = response.ipAddresses.iterator(); iter.hasNext();) {
            ...
            HashMap item = new HashMap();    
            item.put("id", data.publicIpAddressId);
            item.put("networkName", Network.findByReferenceId(data.publicIpAddressAssociatedNetworkid)?.name);
            item.put("networkId", Network.findByReferenceId(data.publicIpAddressAssociatedNetworkid)?.id);
            item.put("networkReferenceId", Network.findByReferenceId(data.publicIpAddressAssociatedNetworkid)?.referenceId);
            item.put("ip", data.publicIpAddress);
            item.put("isSourceNat", data.publicIpAddressIsSourceNat);
            item.put("isStaticNat", data.publicIpAddressIsStaticNat);
            item.put("state", data.publicIpAddressState);
            item.put("vmDisplayName", data.publicIpAddressVirtualMachineDisplayName);
            item.put("vmId", VirtualMachine.findByReferenceId(data.publicIpAddressVirtualMachineId)?.id);
            item.put("vlanId", data.publicIpAddressVlanId);
            item.put("vlanName", data.publicIpAddressVlanName);
            item.put("zone", Zone.findByReferenceZoneId(data.publicIpAddressZoneId)?.aliasName);
            item.put("vpnEnabled", RemoteAccessVpn.findByPublicIPAddress(UserIPAddress.findByIpReferenceId(data.publicIpAddressId))? true: false);
            ...
            item.put("vpcId",  UserIPAddress.findByIpReferenceId(data.publicIpAddressId)?.vpc?.referenceId);
            item.put("vpcName",  UserIPAddress.findByIpReferenceId(data.publicIpAddressId)?.vpc?.name); 
    ...
    def getNetwork(id) {
        ...
        for(Iterator <NetworkResponse> iter = response.networks.iterator(); iter.hasNext();) {
            ...
            item.put("netmask", data.networkNetmask);
        } 
        item.put("referenceId", network.referenceId);
        item.put("id", network.id);
        item.put("name", network.name);
    ...
    def getInternalLoadBalancerList(loadBalancingId, networkId) {
        try {   
            ...
            HashMap<String,String> optional = new HashMap<String,String>();
            optional.put("account", new String(networkAccout.account.userName));
            ...
            for(Iterator <LoadBalancersResponse> iter = response.loadBalancerLists.iterator(); iter.hasNext();) { 
                ...
                HashMap item = new HashMap();
                item.put("id", data.lbRuleId);
                item.put("algorithm", data.algorithm);
                item.put("description", data.description);
                item.put("loadBalancerName", data.loadBalancerName);
                ...
                for(Iterator <LoadBalancerInstanceResponse> vmlbitter1 = data.loadBalancerInstances.iterator(); vmlbitter1.hasNext();) { 
                    ...
                    def vm1 = VirtualMachine.findByReferenceId(vmlb1.instanceId)
                    vmListString += vmListString.size() == 0 ? vmListString : ",";
