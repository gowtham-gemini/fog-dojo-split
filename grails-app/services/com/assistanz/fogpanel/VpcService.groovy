...
class VpcService {
        ...
        try {
            ...
            if(role.iterator().next() == "ROLE_ADMIN" ) {               
                ...
                for(Iterator <VpcResponse> iter = response.vpcs.iterator(); iter.hasNext();) { 
                    def data = iter.next();
                    VPC vpc = VPC.findByReferenceId(data.vpcId)
                    if(vpc) {
    ...
    def listVPCOffering() {
        ...
        try {
            ...
            for(Iterator <VpcOfferingResponse> iter = response.vpcOfferings.iterator(); iter.hasNext();) {
                ...
                VPCOffering vpcOffering = VPCOffering.findByReferenceId(data.vpcId)
                if(!vpcOffering) {
                    vpcOffering = new VPCOffering()
                    vpcOffering.referenceId = data.vpcId 
                    vpcOffering.name = data.vpcOfferingName
                ...
                for(Iterator <ServiceResponse> serviceIter = data.services.iterator(); serviceIter.hasNext();) {
                    ...
                    vpcOfferServiceList.service = serviceData.serviceName
    ...
    //            String vpcOfferingId, String zoneId, HashMap<String, String> optional)
            ...
            VPC vpc = new VPC()
            vpc.referenceId = response.vpcId
            vpc.name = requestData.name
    ...
    def getVpcIpAddressesList(zoneReferenceId, vpcId) {
        try {
            ...
            for(Iterator <IpAddressResponse> iter = response.ipAddresses.iterator(); iter.hasNext();) { 
                ...
                def loadBalancerList = LoadBalancer.findAllByUserIPAddress(UserIPAddress.findByIpReferenceId(data.publicIpAddressId))
                def pfList = PortForwarding.findAllByUserIPAddress(UserIPAddress.findByIpReferenceId(data.publicIpAddressId))                                
                def vmString = ""
                ...
                if(data.publicIpAddressIsStaticNat == true || data.publicIpAddressIsStaticNat == "true") {
                    boolean blnExists = vmListMap.containsKey(data.publicIpAddressVirtualMachineId);
                    def staticVm = VirtualMachine.findByReferenceId(data.publicIpAddressVirtualMachineId)
                    if(blnExists.toString() == "false" || blnExists == false) {                        
                        vmList = staticVm.displayName
                    }
                    vmListMap.put(data.publicIpAddressVirtualMachineId ,"referenceId");
                }
                item.put("vms", vmList);                                
                item.put("id", data.publicIpAddressId);
                item.put("result", "OK");                
                item.put("referenceId", data.publicIpAddressId);                
                item.put("associateNeworkReferenceId", data.publicIpAddressAssociatedNetworkid);
                item.put("network", Network.findByReferenceId(data.publicIpAddressAssociatedNetworkid)?.name);
...
//            log.info("VPC: ${network.referenceId} Created added for account: ${user.account}") 
    ...
    def listS2SVPN(zoneReferenceId, vpcId) {
         ...
         try {   
            ...
            for(Iterator <VpnGatewayResponse> iter = response.vpnGateways.iterator(); iter.hasNext();) { 
                def data = iter.next();                
                S2SVPNGateways s2svpn = S2SVPNGateways.findByReferenceId(data.vpnGatewayId)
                if(!s2svpn) {
                    s2svpn = new S2SVPNGateways()
                    s2svpn.vpc = VPC.findByReferenceId(data.vpcId)  
                    s2svpn.referenceId = data.vpnGatewayId.toString()
    ...
    def acquireIpForVpc(requestData) {
        ...
        try {   
            ...
            if(userip && !invoice) {
                ...
                item.put("ipId", response.publicIpAddressId);
    ...
    def getVPCTopology(zoneReferenceId, vpcId) {
        ...
        for(def vpc: vpcList) {
            ...
            acloptional.put("account", new String(user.account.userName));
            ...
            Console.print("db tier size" + dbTier.size())
