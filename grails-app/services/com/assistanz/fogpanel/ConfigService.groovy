...
import com.assistanz.cloud.cloudstack.configuration.CSConfigurationService
...
class ConfigService {
    ...
    def setDefaultLanguage(String requestBody) {
        try {
            ...
            Config defaultDate =  Config.findByName(Config.DATE_FORMAT);
            ...
            defaultDate.value = requestData.dateFormat;
            defaultDate.save(flush: true);
    ...
    def getMailConfig() {
        ...
        item.put("applicationUrl", ApplicationHolder.getApplication().config.cloudstack.applicationUrl)
    ...
    def setRetailManagementConfig(String requestBody) {
        try {
            ...
            enableThresholdLimit.value = requestData.enableThresholdLimit;
            enableThresholdLimit.save(flush: true);
            
            if(requestData.enableThresholdLimit == "true" || requestData.enableThresholdLimit == true) {
                Config retailInstanceLimit =  Config.findByName(Config.RETAIL_INSTANCE_LIMIT);
                retailInstanceLimit.value = requestData.retailInstanceLimit;
                retailInstanceLimit.save(flush: true);

                Config retailStorageLimit =  Config.findByName(Config.RETAIL_STORAGE_LIMIT);
                retailStorageLimit.value = requestData.retailStorageLimit;
                retailStorageLimit.save(flush: true);

                Config retailSnapshotLimit =  Config.findByName(Config.RETAIL_SNAPSHOT_LIMIT);
                retailSnapshotLimit.value = requestData.retailSnapshotLimit;
                retailSnapshotLimit.save(flush: true);
                
                Config retailNetworkLimit =  Config.findByName(Config.RETAIL_NETWORK_LIMIT);
                retailNetworkLimit.value = requestData.retailNetworkLimit;
                retailNetworkLimit.save(flush: true);

                Config retailPublicIPLimit =  Config.findByName(Config.RETAIL_PUBLIC_IP_LIMIT);
                retailPublicIPLimit.value = requestData.retailPublicIPLimit;
                retailPublicIPLimit.save(flush: true);

                Config retailVpcLimit =  Config.findByName(Config.RETAIL_VPC_LIMIT);
                retailVpcLimit.value = requestData.retailVpcLimit;
                retailVpcLimit.save(flush: true);

                Config retailCpuLimit =  Config.findByName(Config.RETAIL_CUP_LIMIT);
                retailCpuLimit.value = requestData.retailCpuLimit;
                retailCpuLimit.save(flush: true);

                Config retailMemoryLimit =  Config.findByName(Config.RETAIL_MEMORY_LIMIT);
                retailMemoryLimit.value = requestData.retailMemoryLimit;
                retailMemoryLimit.save(flush: true);

                Config primaryStorageLimit =  Config.findByName(Config.RETAIL_PRIMARY_STORAGE_LIMIT);
                primaryStorageLimit.value = requestData.primaryStorageLimit;
                primaryStorageLimit.save(flush: true);

                Config secondaryStorageLimit =  Config.findByName(Config.RETAIL_SECONDARY_STORAGE_LIMIT);
                secondaryStorageLimit.value = requestData.secondaryStorageLimit;
                secondaryStorageLimit.save(flush: true);
                        
            } else {
                Config retailInstanceLimit =  Config.findByName(Config.RETAIL_INSTANCE_LIMIT);
                retailInstanceLimit.value = "-1";
                retailInstanceLimit.save(flush: true);

                Config retailStorageLimit =  Config.findByName(Config.RETAIL_STORAGE_LIMIT);
                retailStorageLimit.value = "-1";
                retailStorageLimit.save(flush: true);

                Config retailSnapshotLimit =  Config.findByName(Config.RETAIL_SNAPSHOT_LIMIT);
                retailSnapshotLimit.value = "-1";
                retailSnapshotLimit.save(flush: true);

                Config retailPublicIPLimit =  Config.findByName(Config.RETAIL_PUBLIC_IP_LIMIT);
                retailPublicIPLimit.value = "-1";
                retailPublicIPLimit.save(flush: true);

                Config retailVpcLimit =  Config.findByName(Config.RETAIL_VPC_LIMIT);
                retailVpcLimit.value = "-1";
                retailVpcLimit.save(flush: true);

                Config retailCpuLimit =  Config.findByName(Config.RETAIL_CUP_LIMIT);
                retailCpuLimit.value = "-1";
                retailCpuLimit.save(flush: true);

                Config retailMemoryLimit =  Config.findByName(Config.RETAIL_MEMORY_LIMIT);
                retailMemoryLimit.value = "-1";
                retailMemoryLimit.save(flush: true);

                Config primaryStorageLimit =  Config.findByName(Config.RETAIL_PRIMARY_STORAGE_LIMIT);
                primaryStorageLimit.value = "-1";
                primaryStorageLimit.save(flush: true);

                Config secondaryStorageLimit =  Config.findByName(Config.RETAIL_SECONDARY_STORAGE_LIMIT);
                secondaryStorageLimit.value = "-1";
                secondaryStorageLimit.save(flush: true);
            }
