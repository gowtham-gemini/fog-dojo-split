...
class NotificationService {
   ...
   def eventList() {
        ...
        for(Iterator<EventsResponse> iter = response.events.iterator(); iter.hasNext();) {
            def data = iter.next();
            def account = Account.findByUserName(data.accountName)
            if(account) {
                ...
                eventItem.put("id", data.eventId);
                eventItem.put("description", data.eventDescription);
                eventItem.put("level", data.eventLevel);
                eventItem.put("type", data.EventType);
                eventItem.put("state", data.eventState);
                eventItem.put("date", dateFormat.format(eventDate.getTime()).toString());
                eventItem.put("account", account.userName);
   ...
   def csAlertsList() {
            ...
            for(Iterator<AlertResponse> iter = response.alerts.iterator(); iter.hasNext();) {
                ...
                Date date = sdf.parse(data.alertSent);
                def cal = Calendar.getInstance()
                cal.setTime(date);
                Date eventDate = cal.getTime();  
                
                HashMap<String,String> eventItem = new HashMap<String,String>();  
                eventItem.put("id", data.alertId);
                eventItem.put("description", data.alertDescription);
                eventItem.put("date", dateFormat.format(eventDate.getTime()).toString());
...
//                event.put("eventType", item.eventType);
    ...
    def send(String toMail, String mailSubject, Map root, String templateName) {
        try {    
            ...
            def templateString = mailTemplateManager.getTemplate(root, mailTemplateURL, templateName, language)
            def templateSubject = mailTemplateManager.getTemplate(root, mailTemplateURL+"/subject", mailSubject, language)
