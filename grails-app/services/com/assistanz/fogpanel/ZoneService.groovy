...
//                networkOfferResult = NetworkOffer.findAllWhere(deleted: false, zone: Zone.findByReferenceZoneId(zoneReferenceId));
            ...
            for(int i =0 ; i < computingOfferResult.size(); i++) {
                ...
                if(item) {
                    ...
                    ArrayList computingOfferCostArrayList = new ArrayList(); 
                    for(def computingOfferZoneCost: item.computingOfferZoneCosts) {
                        HashMap computingOfferCostItem = new HashMap();  
                        computingOfferCostItem.put("cost", computingOfferZoneCost.cost)
                        computingOfferCostItem.put("zone", computingOfferZoneCost.zone.aliasName)
                        computingOfferCostItem.put("zoneReferenceId", computingOfferZoneCost.zone.referenceZoneId)
                        
                        computingOfferCostArrayList.add(computingOfferCostItem)
                    }
                    computingOfferItem.put("cost", computingOfferCostArrayList)     
            ...
            for(int i =0 ; i < diskOfferResult.size(); i++) {
                ...
                if(item) {
                    ...
                    ArrayList diskOfferCostArrayList = new ArrayList(); 
                    for(def diskOfferZoneCost: item.diskOfferZoneCosts) {
                        HashMap diskOfferCostItem = new HashMap();  
                        diskOfferCostItem.put("cost", diskOfferZoneCost.cost)
                        diskOfferCostItem.put("zone", diskOfferZoneCost.zone.aliasName)
                        diskOfferCostItem.put("zoneReferenceId", diskOfferZoneCost.zone.referenceZoneId)
                        
                        diskOfferCostArrayList.add(diskOfferCostItem)
                    }
                    diskOfferItem.put("cost", diskOfferCostArrayList)     
