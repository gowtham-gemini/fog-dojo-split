...
//        }
        ...
        for(int i=0; i < accountList.size(); i++) { 
            ...
            try {
                ...
                if(invoice) {
                    ...
                    usageResponseForAccount(account, startDate)
                }
    ...
    def usageResponseForAccount(account, startDate) {       
        try {
...
//                throw ex;
     ...
     */
    def loopForUsageResponse(response, account) {
        try {     
    ...
    private addUsageRecord(offeringUsage, data, startTimestamp, endTimestamp) {
        try {  
            ...
            def usageCalcType = Config.findByName(Config.USAGE_BILLING_CALC_TYPE).value
            ...
            if(usageCalcType == "ACTUAL") {
               ...
               offeringUsage.hours = Math.ceil(Double.parseDouble(data.rawUsage)); 
            }
            ...
            if(data.usageType == "1") {
               ...
               if(offering) {
                    ...
                    if(usageCalcType == "ACTUAL") {
                        offeringUsage.cost = ComputingOfferZoneCost.findWhere(computingOffer:offering, zone:Zone.findByReferenceZoneId(data.zoneId)).cost * Double.parseDouble(data.rawUsage);
                    } else {
                       offeringUsage.cost = ComputingOfferZoneCost.findWhere(computingOffer:offering, zone:Zone.findByReferenceZoneId(data.zoneId)).cost * Math.ceil(Double.parseDouble(data.rawUsage)); 
                    }
               ...
               invoiceItem = vms[0]
                ...
                if(offering) {
                    if(usageCalcType == "ACTUAL") {
                         offeringUsage.cost = DiskOfferZoneCost.findWhere(diskOffer:offering, zone:Zone.findByReferenceZoneId(data.zoneId)).cost * size * Double.parseDouble(data.rawUsage);
                    } else {
                       offeringUsage.cost = DiskOfferZoneCost.findWhere(diskOffer:offering, zone:Zone.findByReferenceZoneId(data.zoneId)).cost * size * Math.ceil(Double.parseDouble(data.rawUsage)); 
                    } 
                }
...
//                invoiceItem = InvoiceItem.findWhere(invoice: invoice, referenceItemId:data.usageId, billableItem: diskbillableItem)
                ...
                if(snapshotCost) {
                    ...
                    if(usageCalcType == "ACTUAL") {
                       ...
                       offeringUsage.cost = snapshotCost.cost * size * Math.ceil(Double.parseDouble(data.rawUsage));
                    } 
                ...
                if(vmSnapshotCost) {
                    if(usageCalcType == "ACTUAL") {
                       ...
                       offeringUsage.cost = vmSnapshotCost.cost * Math.ceil(Double.parseDouble(data.rawUsage));
                    } 
        ...
        try { 
            ...
            if(data.usageType == "1") {
                ...
                if(offering) {
                    ...
                    newInvoiceItem.usageUnitPrice = offeringUsage.cost
