    ...
    def list() {
        try { 
            ...
            for(Iterator<SecurityGroupResponse> iter = response.securityGroups.iterator(); iter.hasNext();) {
                ...
                securityGroup.put("securityGroupId",data.securityGroupId);
                securityGroup.put("securityGroupName", data.securityGroupName);
                def virtualMachine = VirtualMachine.findAllBySecurityGroupReferenceId(data.securityGroupId);
                def vmListArray = []
...
//                    def securityGroupDeleteResponse = securityGroupServer().deleteSecurityGroup(secOptional);
    ...
    def create(String requestBody) {
        try { 
            ...
            listOptional.put("account", new String(user.account.userName));
            ...
            if(listSecurityGroupResponse.securityGroups.size() == 0) {
                ...
                securityGroupItem.put("securityGroupId", listSecurityGroupResponse.securityGroups[0].securityGroupId);
                securityGroup.add(securityGroupItem);
            }
            log.info("Firewall created ${result}, firewall: ${response.securityGroupId}")   
            ...
            if(jobResponse.asychronousJobStatus == "0") {
                ...
                if(requestData.ruleType == GeneralConstants.INGRESS) {                    
                    for(Iterator<SecuritygroupJobResultResponse> ingressResponse = jobResponse.ingressRules.iterator(); ingressResponse.hasNext();) {
                        ...
                        item.put("securityGroupRuleId", data.securityGroupRuleId);
