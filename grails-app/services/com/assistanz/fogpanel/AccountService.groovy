...
//            licenseValidationService.authorizeAction(FogPanelService.ACCOUNT_SIGNUP)
            ...
            Account.withTransaction {
                ...
                account.userName =  requestData.email
                    ...
                    if (user.validate()) {
                        def response =
                                csAccService.createAccount("0", account.email, account.firstName,
                                        account.lastName, account.userName, requestData.password, null);
                        
                        println("***** persisting uuid"+response.userId);
                        user.uuid = response.userId;
                        ...
                        Map tempalteMap = notificationService.getDefaultMailTemplateMap()
                        tempalteMap.put("userMail" , account.userName)       
                        ...
                        if(requestData.accountType == "RETAIL" && enableThresholdLimit.value == "true") { 
                            ...
                            pStorageLimitoptional.put("account", new String(account.userName));
                            ...
                            sStorgeLimitoptional.put("account", new String(account.userName));
                            ...
                            memoryLimitoptional.put("account", new String(account.userName));
                            ...
                            cpuLimitoptional.put("account", new String(account.userName));
                            ...
                            vpcLimitoptional.put("account", new String(account.userName));
                            ...
                            networkLimitoptional.put("account", new String(account.userName));
                            ...
                            VMoptional.put("account", new String(account.userName));
                            ...
                            stoOptional.put("account", new String(account.userName));
                            ...
                            snapOptional.put("account", new String(account.userName));
    ...
    def list(String email, String userName) {
         try {  
            ...
            if((email == "null" || email == null) && (userName == "null" || userName == null)) {                      
                ...
                if(role.iterator().next() == "ROLE_ADMIN" ) {
                   def result = Account.findAll();                       
                    ...
                    for(int i=0; i < result.size(); i++) {
                        def item = result[i]; 
                        if(item.userName != user.account.userName) {
                            ...
                            def lockuser = User.findByUsername(item.userName)
                            if(lockuser) {
                ...
                def result = Account.findByUserName(userName);
    ...
    def getCurrentAccount() {
       ...
       def currency = ApplicationHolder.getApplication().config.billing.defaultCurrency
        try {
            ...
            optional.put("name", new String(caccount.userName));
            
            def response = csAccService.listAccounts(optional);
            
            for(Iterator<AccountResponse> iter = response.accounts.iterator(); iter.hasNext();) {
                def data = iter.next();
                if(data.accountId != null) { 
                    caccount.uuid = data.accountId;
                    caccount.save(flush:true);
//                    if (!caccount.save()) {
//                        caccount.errors.allErrors.each { Console.print(it) }
//                    }
                }
            }
            
            
//            def result = Account.findByUserName(name);
            def userDateFormat = ""
            ...
            Console.print("userDateFormat" + userDateFormat)
            ...
            HashMap accountItem = new HashMap();
            Console.print("Date" + dateFormat.format(result.account.signUpDate).toString())
                accountItem.put("id", result.account.id);
                accountItem.put("dateFormat", result.dateFormat);                
                accountItem.put("userName", result.account.userName);
...
//            mailTemplate.hasSignature = requestData.hasSignature;
    ...
    def sendSettingsInfoMail(String requestBody) {
        try {        
            ...
            if(user) {  
                ...
                notificationService.send(user.username.toString(), subject, tempalteMap, templateName)
                item.put("Result",  GeneralConstants.RESULT_SUCCESS);
    ...
    def enableAccount(requestBody) {
        ...
        try {  
            ...
            if(role.iterator().next() == "ROLE_ADMIN" ) {
                ...
                def log = new Log()
                log.account = account
                log.user = User.findByUsername(account.userName)
                log.date = time
                log.type = LogType.values()[3]
                log.mailSend = true
                log.description = "Account " + account.userName + " is enabled"
                log.save(flush: true);               
                Map tempalteMap = notificationService.getDefaultMailTemplateMap()       
                tempalteMap.put("firstName", account.firstName)                      
    ...
    def suspendAccount(requestBody) {
        ...
        try {  
            ...
            if(role.iterator().next() == "ROLE_ADMIN" ) {
                ...
                log.mailSend = true
                log.description = "Account " + account.userName + "is suspended due to" + requestData.suspendTextContent
                log.save(flush: true);                                                
                Map tempalteMap = notificationService.getDefaultMailTemplateMap()    
                tempalteMap.put("firstName", account.firstName)
    ...
    def disableAccount(requestBody) {
        ...
        try {  
            ...
            if(role.iterator().next() == "ROLE_ADMIN" ) {
                ...
                log.mailSend = false
                log.description = "Account " + account.userName + "is disabled"
                log.save(flush: true);
    ...
    def processForCancelAccount(account) { 
        ...
        try {  
            ...
            log.type = LogType.values()[3]
            log.mailSend = true
            log.description = "Account " + account.userName + " is cancelled"
            log.save(flush: true);                                    
            item.put("result",  GeneralConstants.RESULT_SUCCESS);
    ...
    def removeDiskForClosedAccount(account) {
        ...
        log.info("Cancel Account Clean last run for account:${account.userName}");
    }
...
//        def currentUser = springSecurityService.currentUser 
        ...
        if(role.iterator().next() == "ROLE_ADMIN" ) {
            ...
            log.type = LogType.values()[3]
            log.mailSend = false
            log.description = "Account " + account.userName + " is locked"
            log.save(flush: true);
...
//        def currentUser = springSecurityService.currentUser 
        ...
        if(role.iterator().next() == "ROLE_ADMIN" ) {
            ...
            log.type = LogType.values()[3]
            log.mailSend = true
            log.description = "Account " + account.userName + " is UnLocked"
            log.save(flush: true);
    ...
    def createSSHKey(requestBody) {
        ...
        try {
            ...
            HashMap<String,String> optional = new HashMap<String,String>();
            optional.put("account", user.account.userName);
    ...
    def getCloudResourceStat(id) {
        ...
        if(role.iterator().next() == "ROLE_ADMIN" ) {
            ...
            user = User.findByUsername(account.userName)
        } else if(role.iterator().next() == "ROLE_FREE_USER" ||"ROLE_PAID_USER") {               
