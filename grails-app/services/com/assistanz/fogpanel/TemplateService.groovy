...
class TemplateService {
    ...
    def pullTemplateFromCloudStack() {
        ...
        for(Iterator<TemplateResponse> iter = response.templates.iterator(); iter.hasNext();) {
            ...
            def aviliableZone = Zone.findByReferenceZoneId(data.templateZoneId)
            if(!template && aviliableZone) {
                
                template  = new Template();            
                template.name = data.templateName;
                template.description = data.templateName;
                ...
                template.detailedDescription = data.templateDisplayText;
                ...
                if(data.templateSize == null || data.templateSize == "null") {     
                    ...
                    def tempsize = Double.parseDouble(data.templateSize);
                ...
                template.addToZones(Zone.findByReferenceZoneId(data.templateZoneId));
                ...
                boolean blnExists = temZoneList.containsKey(data.templateZoneId);
                if(blnExists.toString() == "false" || blnExists == false) {
                    def zone = Zone.findWhere(referenceZoneId:data.templateZoneId) 
                    if(zone) {
        ...
        try {                 
            ...
            for(Iterator<TemplateResponse> iter = response.templates.iterator(); iter.hasNext();) {
                ...
                if(template) {
                    ...
                    if(data.templateSize == null || data.templateSize == "null") {                        
                        template.size = 0
                        if(data.templateStatus == "" || data.templateStatus == "null" || data.templateStatus == null) {
                          template.status = "-";      
                        } else {
                            template.status = data.templateStatus;    
                        }
...
//            Console.print("ex" + requestData.extractable)
            ...
            oldTemplate.name = response.templateName;
            oldTemplate.description = response.templateDisplayText;
