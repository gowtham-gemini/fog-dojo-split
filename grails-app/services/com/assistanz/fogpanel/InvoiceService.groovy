            ...
            Config billingPeriodfixedDay =  Config.findByName(Config.BILLING_PERIOD_FIXED_DAY);
                ...
                for(def invoice: monthlyInvoiceList) {
                    ...
                    HashMap<String,String> cus = new HashMap<String,String>();                 
                        cus.put("id", item.id);
                        cus.put("account", item.invoice.account.userName);
            ...
            if(account.cardVerified == true || account.cardVerified == "true") {
                ...
                if((creditCardEnabled.value == "true" || creditCardEnabled.value == true) && PaymentGateways.get(1).status == "ENABLE") {
                    ...
                    if(((account.cardExpiryMonth - 1.0) == Double.parseDouble(Integer.toString(month)))  && (account.cardExpiryYear == Double.parseDouble(Integer.toString(year)))) {
                        ...
                        dbLog.user = User.findByUsername(account.userName)
                        dbLog.date = time
                        dbLog.type = LogType.values()[2]
                        dbLog.mailSend = true
                        dbLog.description = "Account " + account.userName + "is disabled due your credit card is about to expire"
                        ...
                        tempalteMap.put("user", User.findByUsername(account.userName))
        ...
        try { 
            ...
            if((account.totalPayable - account.credit) >= account.creditLimit) {
                if(account.status.name().toString() == "ACTIVE") {
                       ...
                       dbLog.description = "Account " + account.userName + " is Disabled due to over credit limit"
                       dbLog.save(flush: true);
                       log.info("Account disabled for account: ${account.id} by cron jobs") 
//                    def mailtemplate = MailTemplate.findByName("suspended")
//                    def setUserName = mailtemplate.content.replaceAll("userName", account.userName)
...
//                    notificationService.send(account.email.toString(), "Invoice Notification", message.toString())
                    ...
                    dbLog.mailSend = false
                    dbLog.description = "Account " + account.userName + " is reached " + creditLimitNotificationLevel2Percentage.value + "% of credit limit";
                    dbLog.save(flush: true);
...
//                    notificationService.send(account.email.toString(), "Invoice Notification", message.toString())
                    ...
                    dbLog.mailSend = false
                    dbLog.description = "Account " + account.userName + " is reached " + creditLimitNotificationLevel3Percentage.value + "% of credit limit";
                    dbLog.save(flush: true);
...
//                    notificationService.send(account.email.toString(), "Invoice Notification", message.toString())
                    ...
                    dbLog.description = "Account " + account.userName + " is reached " + creditLimitNotificationLevel1Percentage.value + "% of credit limit";
                    dbLog.save(flush: true);
...
//                    notificationService.send(account.email.toString(), "Invoice Notification", message.toString())
                    dbLog.creditLimitPercentage = creditLimitNotificationLevel2Percentage.value
                    dbLog.description = "Your account " + account.userName + " is reached " + creditLimitNotificationLevel2Percentage.value + "% of credit limit";
                    dbLog.description = "Account " + account.userName + " is reached " + creditLimitNotificationLevel2Percentage.value + "% of credit limit";
                    dbLog.save(flush: true);
                    account.creditLimitLevel = "2"
                    account.save(flush: true);
...
//                    notificationService.send(account.email.toString(), "Invoice Notification", message.toString())
    ...
    def addRefundAmount(requestBody) {
        ...
        try { 
            ...
            tempalteMap.put("user", User.findByUsername(account.email))
            notificationService.send(account.userName.toString(), "refund.subject.ftl", tempalteMap, "refund.ftl")  
