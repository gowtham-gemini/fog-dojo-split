...
//        JSONObject json = new JSONObject();
        ...
        DateFormat givenDateFormater = new SimpleDateFormat(dateFormatValue);
                ...
                for(def billableItem : billableItemList) {
                    ...
                    for(def invoiceItem: invoiceItemList) {
                        if(invoiceItem.totalAmount != 0) {
                            ...
                            item.put("account", invoice.account.userName);
...
//                    if(acc.userName == itemArray.account) {
//                       accCount +=1 
//                    }
//                }
//                if(accCount != 0) {
//                   planCountMap.put(acc.userName, accCount);  
//                }
//            }
    ...
    def getComputePlanCostForAccount(planList, billableItem, billableItemData)  {
        ...
        if(billableItemData.forAccount =="ALL") {
            ...
            for(def account: accountList) {
                ...
                if(totalAmount !=0) {
                    unsortMap.put(account.userName, totalAmount);
                }
            }
...
//            Console.print("invoiceItemListArray" + invoiceItemListArray);
    ...
    def getServiceChangeLogHistory() {
        ...
        def serviceCostChangeLogList = ServiceCostChangeLog.listOrderByChangedDate()
        
        ArrayList<ArrayList<String>> invoiceItemListArray = new ArrayList<ArrayList<String>>(); 
                
        for(def serviceCostChangeLog : serviceCostChangeLogList) {
            
            HashMap item = new HashMap()                       
            
            if(serviceCostChangeLog.serviceName.name() == "COMPUTE_OFFER") {
                item.put("serviceName", "COMPUTE OFFER")
                item.put("itemName", serviceCostChangeLog?.computingOfferZoneCost?.computingOffer?.name)
                item.put("zone", serviceCostChangeLog?.computingOfferZoneCost?.zone.aliasName)
                item.put("unit", "/HR")
                
            } else if (serviceCostChangeLog.serviceName.name() == "DISK_OFFER") {
                item.put("serviceName", "DISK OFFER")
                item.put("itemName", serviceCostChangeLog.diskOfferZoneCost?.diskOffer?.name)
                item.put("zone", serviceCostChangeLog?.diskOfferZoneCost?.zone.aliasName)
                item.put("unit", "/GB/HR")
                
            } else if (serviceCostChangeLog.serviceName.name() == "MISC_OFFER") {
                item.put("serviceName","MISC OFFER")
                item.put("itemName", serviceCostChangeLog.miscellaneousOfferZoneCost?.miscellaneousOffer?.name)
                item.put("zone", serviceCostChangeLog?.miscellaneousOfferZoneCost?.zone.aliasName)
                if(serviceCostChangeLog.miscellaneousOfferZoneCost?.miscellaneousOffer?.id == 1) {
                    item.put("unit", "/GB/Month")
                } else if (serviceCostChangeLog.miscellaneousOfferZoneCost?.miscellaneousOffer?.id == 2) {
                    item.put("unit", "/IP/Month")
                } else if (serviceCostChangeLog.miscellaneousOfferZoneCost?.miscellaneousOffer?.id == 3) {
                    item.put("unit", "/GB/HR")
                } else if (serviceCostChangeLog.miscellaneousOfferZoneCost?.miscellaneousOffer?.id == 4) {
                    item.put("unit", "/GB/HR")
                } else if (serviceCostChangeLog.miscellaneousOfferZoneCost?.miscellaneousOffer?.id == 5) {
                    item.put("unit", "/Policy/Month")
                } else if (serviceCostChangeLog.miscellaneousOfferZoneCost?.miscellaneousOffer?.id == 6) {
                    item.put("unit", "/Rule/Month")
                } else if (serviceCostChangeLog.miscellaneousOfferZoneCost?.miscellaneousOffer?.id == 7) {
                    item.put("unit", "/Connection/Month")
                }
            }
            item.put("oldCost", serviceCostChangeLog.oldCost)
            item.put("newCost", serviceCostChangeLog.newCost)
            item.put("id", serviceCostChangeLog.id)
            item.put("changedDate", outPutFormater.format(serviceCostChangeLog.changedDate))
            item.put("user", serviceCostChangeLog.user.username)
