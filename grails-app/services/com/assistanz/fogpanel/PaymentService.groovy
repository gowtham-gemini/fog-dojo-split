...
class PaymentService {
    ...
    def makeAutomatedPaymentAttempt1() {
        ...
        log.info("AutoPayment Cron Completed");
    }
    ...
    def makeAutomatedPayment(account, dueInvoice, attempt) {
        ...
        try {
            ...
            //            between("paymentDate", previousInvoice.date, previousInvoice.dueDate)   
            ...
            if(paymentTotalAmount > 0) {
                ...
                log.info("auto payment attempt ${attempt} for account: ${account.id}, amount: ${payAmount}");
            ...
            account.save(flush:true);
            log.info("auto payment attempt ${attempt} for account: ${account.id}, amount: ${paymentTotalAmount} is failed due to ${ex}");
            throw ex;
        }  
    }
    ...
    def count(forMonth) {
        ...
        if(forMonth == "Today") {
            ...
            for(int i=0; i < todayPaymentList.size(); i++) { 
                ...
                payment.put("account", item.account.userName);
            ...
            for(int i=0; i < currentMonthPayments.size(); i++) { 
                ...
                payment.put("account", currentMonthItem.account.userName);
            ...
            for(int i=0; i < preMonthPayments.size(); i++) { 
                ...
                payment.put("account", preMonthItem.account.userName);
    ...
    def manualEntryForPayment(paymentData) {
        ...
        Date date; 
        def dateFormatValue = configService.getDateFormat();
        formatter = new SimpleDateFormat(dateFormatValue);
