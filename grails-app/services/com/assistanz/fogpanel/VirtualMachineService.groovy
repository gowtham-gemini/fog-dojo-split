        ...
        try {            
            ...
            if(role.iterator().next() == "ROLE_ADMIN" ) {                
                ...
                if(volumeReferenceId == null || volumeReferenceId == 'null') {                    
                    ...
                    result = VirtualMachine.findAllWhere(deleted: false, user: user, zone:volume.zone, vpc: null);                     
                }     
                for(int i=0; i < result.size(); i++) {                
                    ...
                    if(volumeList.size() < 5) {        
                        ...
                        instances.put("zoneReferenceId", item.zone.referenceZoneId);
...
//            newVirtualMachine.cluster = newVirtualMachine.computingOffer.cluster;
    ...
    def addSetupCost(invoice, newVirtualMachine) {
        ...
        def setupCost = ComputingOfferZoneCost.findWhere(computingOffer: newVirtualMachine.computingOffer, zone: newVirtualMachine.zone).setupCost
        ...
        if(setupCost != 0) {
            ...
            if(setupBillableItem.enabled == true || setupBillableItem.enabled == "true") {
                ...
                invoiceItem.usageUnitPrice = ComputingOfferZoneCost.findWhere(computingOffer: newVirtualMachine.computingOffer, zone: newVirtualMachine.zone).setupCost
...
//            virtualMachine.cluster = virtualMachine.computingOffer.cluster;
            ...
            if(computingBillableItem.enabled == true || computingBillableItem.enabled == "true") {
                ...
                if(virtualMachine.billingType == "monthly") {
                    ...
                    double monthlyAmount = ComputingOfferZoneCost.findWhere(computingOffer: virtualMachine.computingOffer, zone: virtualMachine.zone).cost * 720.00   
                    ...
                    invoiceItem.usageUnitPrice = ComputingOfferZoneCost.findWhere(computingOffer: virtualMachine.computingOffer, zone: virtualMachine.zone).cost
                    invoiceItem.referenceItemName = "VirtualMachine"
                    invoiceItem.referenceItemId = virtualMachine.referenceId
                    invoiceItem.save(flush: true); 
                }
        ...
        try {
            ...
            if((accountId == "null" || accountId == null) && (status == "null" || status == null) && (zoneReferenceId == "null" || zoneReferenceId == null)) {   
                ...
                for(int i=0; i < instances.size(); i++) { 
                    ...
                    instancesItem.put("hasVpc", hasVpc);                      
