...
import com.assistanz.fogpanel.DiskOfferZoneCost
...
//        VolumeJob.triggerNow()
	...
	JSON.registerObjectMarshaller(MiscellaneousOfferZoneCost) {
        ...
        JSON.registerObjectMarshaller(ComputingOfferZoneCost) {
            def computingOfferZoneCostArray = [:]
            computingOfferZoneCostArray['id'] = it.id      
            computingOfferZoneCostArray['zone'] = it.zone                               
            computingOfferZoneCostArray['cost'] = it.cost
            computingOfferZoneCostArray['setupCost'] = it.setupCost
            computingOfferZoneCostArray['minimumCost'] = it.minimumCost
            computingOfferZoneCostArray['computingOffer'] = it.computingOffer
            computingOfferZoneCostArray['currency'] = currency   
            return computingOfferZoneCostArray
        } 
        
        JSON.registerObjectMarshaller(DiskOfferZoneCost) {
            def diskOfferZoneCostArray = [:]
            diskOfferZoneCostArray['id'] = it.id      
            diskOfferZoneCostArray['zone'] = it.zone                               
            diskOfferZoneCostArray['cost'] = it.cost            
            diskOfferZoneCostArray['minimumCost'] = it.minimumCost
            diskOfferZoneCostArray['computingOffer'] = it.diskOffer
            diskOfferZoneCostArray['currency'] = currency   
            return diskOfferZoneCostArray
        } 
        
        JSON.registerObjectMarshaller(Zone) {
            def zoneCostArray = [:]
            zoneCostArray['referenceZoneId'] = it.referenceZoneId      
            zoneCostArray['name'] = it.name                               
            zoneCostArray['available'] = it.available
            zoneCostArray['aliasName'] = it.aliasName
            zoneCostArray['aliasDescription'] = it.aliasDescription
            zoneCostArray['orderNo'] = it.orderNo
            zoneCostArray['id'] = it.id
            zoneCostArray['networkType'] = it.networkType            
            zoneCostArray['currency'] = currency   
            return zoneCostArray
        } 
