    ...
    def savePassword() {       
        ...
        if(user) {            
            ...
            if(newPassword == "" || confirPasword == "" || newPassword == " " || confirPasword == " ") {              
                ...
                userParams.put("password", params.confirmPassword)
...
//        session['org.springframework.web.servlet.i18n.SessionLocaleResolver.LOCALE'] = new Locale(defaultLanguage.value, defaultLanguage.value)
    ...
    def retailSignup() {
        ...
        def advancedPayments = PaymentGateways.findAllByGatewayType("ADVANCED")
        for(def pay: advancedPayments) {
            if(pay.isDefault == true) {
               enableCard = true;
               break;
            }
        }       
